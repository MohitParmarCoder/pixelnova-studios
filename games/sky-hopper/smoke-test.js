'use strict';
// Headless smoke test for sky-hopper (SkyHopper)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SkyHopper !== 'undefined' && SkyHopper.update) SkyHopper.update(dt);
    if (typeof SkyHopper !== 'undefined' && SkyHopper.draw) SkyHopper.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SkyHopper !== 'undefined' && SkyHopper.tap) SkyHopper.tap(x,y);
...
// init
try {
  if (typeof SkyHopper !== 'undefined' && SkyHopper.init) SkyHopper.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (sky-hopper): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS sky-hopper');
}
