...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('stargrab_best'), 10) || 0; } catch(e) {}
  StarGrab.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); StarGrab.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { StarGrab.update(dt); }
    StarGrab.draw();
    var b = StarGrab.getBest ? StarGrab.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('stargrab_best', String(best)); } catch(e) {} }
