'use strict';
// Headless smoke test for star-grab (StarGrab)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof StarGrab !== 'undefined' && StarGrab.update) StarGrab.update(dt);
    if (typeof StarGrab !== 'undefined' && StarGrab.draw) StarGrab.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof StarGrab !== 'undefined' && StarGrab.tap) StarGrab.tap(x,y);
...
// init
try {
  if (typeof StarGrab !== 'undefined' && StarGrab.init) StarGrab.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (star-grab): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS star-grab');
}
