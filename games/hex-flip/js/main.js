...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('hexflip_best'), 10) || 0; } catch(e) {}
  HexFlip.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); HexFlip.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { HexFlip.update(dt); }
    HexFlip.draw();
    var b = HexFlip.getBest ? HexFlip.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('hexflip_best', String(best)); } catch(e) {} }
