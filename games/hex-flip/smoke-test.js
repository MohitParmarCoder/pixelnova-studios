'use strict';
// Headless smoke test for hex-flip (HexFlip)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof HexFlip !== 'undefined' && HexFlip.update) HexFlip.update(dt);
    if (typeof HexFlip !== 'undefined' && HexFlip.draw) HexFlip.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof HexFlip !== 'undefined' && HexFlip.tap) HexFlip.tap(x,y);
...
// init
try {
  if (typeof HexFlip !== 'undefined' && HexFlip.init) HexFlip.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (hex-flip): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS hex-flip');
}
