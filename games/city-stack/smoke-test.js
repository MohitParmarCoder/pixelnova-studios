'use strict';
// Headless smoke test for city-stack (CityStack)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof CityStack !== 'undefined' && CityStack.update) CityStack.update(dt);
    if (typeof CityStack !== 'undefined' && CityStack.draw) CityStack.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof CityStack !== 'undefined' && CityStack.tap) CityStack.tap(x,y);
...
// init
try {
  if (typeof CityStack !== 'undefined' && CityStack.init) CityStack.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (city-stack): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS city-stack');
}
