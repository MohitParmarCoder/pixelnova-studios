...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('citystack_best'), 10) || 0; } catch(e) {}
  CityStack.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); CityStack.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { CityStack.update(dt); }
    CityStack.draw();
    var b = CityStack.getBest ? CityStack.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('citystack_best', String(best)); } catch(e) {} }
