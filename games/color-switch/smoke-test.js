'use strict';
// Headless smoke test for color-switch (ColorSwitch)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ColorSwitch !== 'undefined' && ColorSwitch.update) ColorSwitch.update(dt);
    if (typeof ColorSwitch !== 'undefined' && ColorSwitch.draw) ColorSwitch.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ColorSwitch !== 'undefined' && ColorSwitch.tap) ColorSwitch.tap(x,y);
...
// init
try {
  if (typeof ColorSwitch !== 'undefined' && ColorSwitch.init) ColorSwitch.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (color-switch): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS color-switch');
}
