'use strict';
// Headless smoke test for block-breaker (BlockBreaker)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BlockBreaker !== 'undefined' && BlockBreaker.update) BlockBreaker.update(dt);
    if (typeof BlockBreaker !== 'undefined' && BlockBreaker.draw) BlockBreaker.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BlockBreaker !== 'undefined' && BlockBreaker.tap) BlockBreaker.tap(x,y);
...
// init
try {
  if (typeof BlockBreaker !== 'undefined' && BlockBreaker.init) BlockBreaker.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (block-breaker): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS block-breaker');
}
