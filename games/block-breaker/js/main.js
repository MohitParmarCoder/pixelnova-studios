...
(function () {
  const VW = 390, VH = 844;
  const BEST_KEY = 'blockbreaker_best';
  ...
  canvas.addEventListener('pointerdown', (e) => {
    e.preventDefault();
    dragging = true;
    BlockBreaker.setPaddleX(toVirtualX(e));
    BlockBreaker.tap();
  });
  ...
  function loop(ts) {
    ...
    prev = ts;
    const sp = 360 * dt;
    if (keys.ArrowLeft || keys.a || keys.A) BlockBreaker.nudge(-sp);
    if (keys.ArrowRight || keys.d || keys.D) BlockBreaker.nudge(sp);
    if (!_settingsOpen) { BlockBreaker.update(dt); }
    BlockBreaker.draw();

    // ── Full overlay: PixelNova branding + Settings/Pause ─────────────────────
    (function drawPNOverlay() {
      var c = canvas.getContext('2d');
      var gs = BlockBreaker.getState ? BlockBreaker.getState() : '';
