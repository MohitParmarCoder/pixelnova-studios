...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('lasermaze_best'), 10) || 0; } catch(e) {}
  LaserMaze.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); LaserMaze.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { LaserMaze.update(dt); }
    LaserMaze.draw();
    var b = LaserMaze.getBest ? LaserMaze.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('lasermaze_best', String(best)); } catch(e) {} }
