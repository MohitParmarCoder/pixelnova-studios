'use strict';
// Headless smoke test for laser-maze (LaserMaze)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof LaserMaze !== 'undefined' && LaserMaze.update) LaserMaze.update(dt);
    if (typeof LaserMaze !== 'undefined' && LaserMaze.draw) LaserMaze.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof LaserMaze !== 'undefined' && LaserMaze.tap) LaserMaze.tap(x,y);
...
// init
try {
  if (typeof LaserMaze !== 'undefined' && LaserMaze.init) LaserMaze.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (laser-maze): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS laser-maze');
}
