...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('whackmole_best'), 10) || 0; } catch(e) {}
  WhackMole.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); WhackMole.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { WhackMole.update(dt); }
    WhackMole.draw();
    var b = WhackMole.getBest ? WhackMole.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('whackmole_best', String(best)); } catch(e) {} }
