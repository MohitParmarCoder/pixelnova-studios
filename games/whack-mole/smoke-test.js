'use strict';
// Headless smoke test for whack-mole (WhackMole)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof WhackMole !== 'undefined' && WhackMole.update) WhackMole.update(dt);
    if (typeof WhackMole !== 'undefined' && WhackMole.draw) WhackMole.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof WhackMole !== 'undefined' && WhackMole.tap) WhackMole.tap(x,y);
...
// init
try {
  if (typeof WhackMole !== 'undefined' && WhackMole.init) WhackMole.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (whack-mole): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS whack-mole');
}
