...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('bombsquad_best'), 10) || 0; } catch(e) {}
  BombSquad.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); BombSquad.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { BombSquad.update(dt); }
    BombSquad.draw();
    var b = BombSquad.getBest ? BombSquad.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('bombsquad_best', String(best)); } catch(e) {} }
