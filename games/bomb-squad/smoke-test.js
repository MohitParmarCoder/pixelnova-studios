'use strict';
// Headless smoke test for bomb-squad (BombSquad)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BombSquad !== 'undefined' && BombSquad.update) BombSquad.update(dt);
    if (typeof BombSquad !== 'undefined' && BombSquad.draw) BombSquad.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BombSquad !== 'undefined' && BombSquad.tap) BombSquad.tap(x,y);
...
// init
try {
  if (typeof BombSquad !== 'undefined' && BombSquad.init) BombSquad.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (bomb-squad): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS bomb-squad');
}
