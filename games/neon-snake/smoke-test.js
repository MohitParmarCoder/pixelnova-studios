'use strict';
// Headless smoke test for neon-snake (NeonSnake)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof NeonSnake !== 'undefined' && NeonSnake.update) NeonSnake.update(dt);
    if (typeof NeonSnake !== 'undefined' && NeonSnake.draw) NeonSnake.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof NeonSnake !== 'undefined' && NeonSnake.tap) NeonSnake.tap(x,y);
...
// init
try {
  if (typeof NeonSnake !== 'undefined' && NeonSnake.init) NeonSnake.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (neon-snake): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS neon-snake');
}
