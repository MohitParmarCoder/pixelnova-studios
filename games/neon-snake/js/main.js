...
(function () {
  ...
  function loop(ts) {
    ...
    if (!_paused) { NeonSnake.update(dt); }
    NeonSnake.draw();
  ...
  (function () {
    ...
    if (elIC) elIC.addEventListener('click', function () { _paused = false; prevTs = performance.now(); if (elIO) elIO.classList.add('hidden'); });
    if (elIO) elIO.addEventListener('click', function (e) { if (e.target === this) { _paused = false; prevTs = performance.now(); elIO.classList.add('hidden'); } });
  })();
