'use strict';
// Headless smoke test for lava-surf (LavaSurf)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof LavaSurf !== 'undefined' && LavaSurf.update) LavaSurf.update(dt);
    if (typeof LavaSurf !== 'undefined' && LavaSurf.draw) LavaSurf.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof LavaSurf !== 'undefined' && LavaSurf.tap) LavaSurf.tap(x,y);
...
// init
try {
  if (typeof LavaSurf !== 'undefined' && LavaSurf.init) LavaSurf.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (lava-surf): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS lava-surf');
}
