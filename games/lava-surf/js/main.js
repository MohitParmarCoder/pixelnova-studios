...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('lavasurf_best'), 10) || 0; } catch(e) {}
  LavaSurf.init(canvas, best);
  ...
  canvas.addEventListener('pointerdown', function (e) {
    e.preventDefault();
    var p = toVirtual(e);
    LavaSurf.tap(p.x, p.y);
  });
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); LavaSurf.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { LavaSurf.update(dt); }
    LavaSurf.draw();
    var b = LavaSurf.getBest ? LavaSurf.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('lavasurf_best', String(best)); } catch(e) {} }
