...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('gridsnake_best'), 10) || 0; } catch(e) {}
  GridSnake.init(canvas, best);
  ...
  canvas.addEventListener('pointerdown', function (e) {
    e.preventDefault();
    var p = toVirtual(e);
    GridSnake.tap(p.x, p.y);
  });
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); GridSnake.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { GridSnake.update(dt); }
    GridSnake.draw();
    var b = GridSnake.getBest ? GridSnake.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('gridsnake_best', String(best)); } catch(e) {} }
