'use strict';
// Headless smoke test for grid-snake (GridSnake)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof GridSnake !== 'undefined' && GridSnake.update) GridSnake.update(dt);
    if (typeof GridSnake !== 'undefined' && GridSnake.draw) GridSnake.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof GridSnake !== 'undefined' && GridSnake.tap) GridSnake.tap(x,y);
...
// init
try {
  if (typeof GridSnake !== 'undefined' && GridSnake.init) GridSnake.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (grid-snake): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS grid-snake');
}
