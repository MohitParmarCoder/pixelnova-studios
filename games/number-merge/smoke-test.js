'use strict';
// Headless smoke test for number-merge (NumberMerge)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof NumberMerge !== 'undefined' && NumberMerge.update) NumberMerge.update(dt);
    if (typeof NumberMerge !== 'undefined' && NumberMerge.draw) NumberMerge.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof NumberMerge !== 'undefined' && NumberMerge.tap) NumberMerge.tap(x,y);
...
// init
try {
  if (typeof NumberMerge !== 'undefined' && NumberMerge.init) NumberMerge.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (number-merge): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS number-merge');
}
