'use strict';
// Headless smoke test for flash-tap (FlashTap)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof FlashTap !== 'undefined' && FlashTap.update) FlashTap.update(dt);
    if (typeof FlashTap !== 'undefined' && FlashTap.draw) FlashTap.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof FlashTap !== 'undefined' && FlashTap.tap) FlashTap.tap(x,y);
...
// init
try {
  if (typeof FlashTap !== 'undefined' && FlashTap.init) FlashTap.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (flash-tap): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS flash-tap');
}
