'use strict';
// Headless smoke test for pattern-repeat (PatternRepeat)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PatternRepeat !== 'undefined' && PatternRepeat.update) PatternRepeat.update(dt);
    if (typeof PatternRepeat !== 'undefined' && PatternRepeat.draw) PatternRepeat.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PatternRepeat !== 'undefined' && PatternRepeat.tap) PatternRepeat.tap(x,y);
...
// init
try {
  if (typeof PatternRepeat !== 'undefined' && PatternRepeat.init) PatternRepeat.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (pattern-repeat): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS pattern-repeat');
}
