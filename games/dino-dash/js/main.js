...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('dinodash_best'), 10) || 0; } catch(e) {}
  DinoDash.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); DinoDash.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { DinoDash.update(dt); }
    DinoDash.draw();
    var b = DinoDash.getBest ? DinoDash.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('dinodash_best', String(best)); } catch(e) {} }
