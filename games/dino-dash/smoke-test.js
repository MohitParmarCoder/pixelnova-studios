'use strict';
// Headless smoke test for dino-dash (DinoDash)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof DinoDash !== 'undefined' && DinoDash.update) DinoDash.update(dt);
    if (typeof DinoDash !== 'undefined' && DinoDash.draw) DinoDash.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof DinoDash !== 'undefined' && DinoDash.tap) DinoDash.tap(x,y);
...
// init
try {
  if (typeof DinoDash !== 'undefined' && DinoDash.init) DinoDash.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (dino-dash): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS dino-dash');
}
