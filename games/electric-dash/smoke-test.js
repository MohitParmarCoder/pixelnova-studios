'use strict';
// Headless smoke test for electric-dash (ElectricDash)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ElectricDash !== 'undefined' && ElectricDash.update) ElectricDash.update(dt);
    if (typeof ElectricDash !== 'undefined' && ElectricDash.draw) ElectricDash.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ElectricDash !== 'undefined' && ElectricDash.tap) ElectricDash.tap(x,y);
...
// init
try {
  if (typeof ElectricDash !== 'undefined' && ElectricDash.init) ElectricDash.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (electric-dash): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS electric-dash');
}
