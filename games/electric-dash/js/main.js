...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('electricdash_best'), 10) || 0; } catch(e) {}
  ElectricDash.init(canvas, best);
  ...
  canvas.addEventListener('pointerdown', function (e) {
    e.preventDefault();
    var p = toVirtual(e);
    ElectricDash.tap(p.x, p.y);
  });
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ElectricDash.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ElectricDash.update(dt); }
    ElectricDash.draw();
    var b = ElectricDash.getBest ? ElectricDash.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('electricdash_best', String(best)); } catch(e) {} }
