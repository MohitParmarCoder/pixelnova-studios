'use strict';
// Headless smoke test for candy-rain (CandyRain)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof CandyRain !== 'undefined' && CandyRain.update) CandyRain.update(dt);
    if (typeof CandyRain !== 'undefined' && CandyRain.draw) CandyRain.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof CandyRain !== 'undefined' && CandyRain.tap) CandyRain.tap(x,y);
...
// init
try {
  if (typeof CandyRain !== 'undefined' && CandyRain.init) CandyRain.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (candy-rain): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS candy-rain');
}
