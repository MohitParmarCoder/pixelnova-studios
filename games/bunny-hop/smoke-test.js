'use strict';
// Headless smoke test for bunny-hop (BunnyHop)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BunnyHop !== 'undefined' && BunnyHop.update) BunnyHop.update(dt);
    if (typeof BunnyHop !== 'undefined' && BunnyHop.draw) BunnyHop.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BunnyHop !== 'undefined' && BunnyHop.tap) BunnyHop.tap(x,y);
...
// init
try {
  if (typeof BunnyHop !== 'undefined' && BunnyHop.init) BunnyHop.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (bunny-hop): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS bunny-hop');
}
