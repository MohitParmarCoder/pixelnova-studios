...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('bunnyhop_best'), 10) || 0; } catch(e) {}
  BunnyHop.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); BunnyHop.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { BunnyHop.update(dt); }
    BunnyHop.draw();
    var b = BunnyHop.getBest ? BunnyHop.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('bunnyhop_best', String(best)); } catch(e) {} }
