'use strict';
// Headless smoke test for tower-build (TowerBuild)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof TowerBuild !== 'undefined' && TowerBuild.update) TowerBuild.update(dt);
    if (typeof TowerBuild !== 'undefined' && TowerBuild.draw) TowerBuild.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof TowerBuild !== 'undefined' && TowerBuild.tap) TowerBuild.tap(x,y);
...
// init
try {
  if (typeof TowerBuild !== 'undefined' && TowerBuild.init) TowerBuild.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (tower-build): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS tower-build');
}
