...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('towerbuild_best'), 10) || 0; } catch(e) {}
  TowerBuild.init(canvas, best);
  ...
  canvas.addEventListener('pointerdown', function (e) {
    e.preventDefault();
    var p = toVirtual(e);
    TowerBuild.tap(p.x, p.y);
  });
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); TowerBuild.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { TowerBuild.update(dt); }
    TowerBuild.draw();
    var b = TowerBuild.getBest ? TowerBuild.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('towerbuild_best', String(best)); } catch(e) {} }
