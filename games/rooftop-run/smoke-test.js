'use strict';
// Headless smoke test for rooftop-run (RooftopRun)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof RooftopRun !== 'undefined' && RooftopRun.update) RooftopRun.update(dt);
    if (typeof RooftopRun !== 'undefined' && RooftopRun.draw) RooftopRun.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof RooftopRun !== 'undefined' && RooftopRun.tap) RooftopRun.tap(x,y);
...
// init
try {
  if (typeof RooftopRun !== 'undefined' && RooftopRun.init) RooftopRun.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (rooftop-run): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS rooftop-run');
}
