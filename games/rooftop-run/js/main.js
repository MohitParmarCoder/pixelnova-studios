...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('rooftoprun_best'), 10) || 0; } catch(e) {}
  RooftopRun.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); RooftopRun.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { RooftopRun.update(dt); }
    RooftopRun.draw();
    var b = RooftopRun.getBest ? RooftopRun.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('rooftoprun_best', String(best)); } catch(e) {} }
