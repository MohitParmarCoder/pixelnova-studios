'use strict';
// Headless smoke test for color-flood (ColorFlood)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ColorFlood !== 'undefined' && ColorFlood.update) ColorFlood.update(dt);
    if (typeof ColorFlood !== 'undefined' && ColorFlood.draw) ColorFlood.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ColorFlood !== 'undefined' && ColorFlood.tap) ColorFlood.tap(x,y);
...
// init
try {
  if (typeof ColorFlood !== 'undefined' && ColorFlood.init) ColorFlood.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (color-flood): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS color-flood');
}
