...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('colorflood_best'), 10) || 0; } catch(e) {}
  ColorFlood.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ColorFlood.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ColorFlood.update(dt); }
    ColorFlood.draw();
    var b = ColorFlood.getBest ? ColorFlood.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('colorflood_best', String(best)); } catch(e) {} }
