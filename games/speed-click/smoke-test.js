'use strict';
// Headless smoke test for speed-click (SpeedClick)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SpeedClick !== 'undefined' && SpeedClick.update) SpeedClick.update(dt);
    if (typeof SpeedClick !== 'undefined' && SpeedClick.draw) SpeedClick.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SpeedClick !== 'undefined' && SpeedClick.tap) SpeedClick.tap(x,y);
...
// init
try {
  if (typeof SpeedClick !== 'undefined' && SpeedClick.init) SpeedClick.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (speed-click): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS speed-click');
}
