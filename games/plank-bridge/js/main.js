...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('plankbridge_best'), 10) || 0; } catch(e) {}
  PlankBridge.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); PlankBridge.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { PlankBridge.update(dt); }
    PlankBridge.draw();
    var b = PlankBridge.getBest ? PlankBridge.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('plankbridge_best', String(best)); } catch(e) {} }
