'use strict';
// Headless smoke test for plank-bridge (PlankBridge)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PlankBridge !== 'undefined' && PlankBridge.update) PlankBridge.update(dt);
    if (typeof PlankBridge !== 'undefined' && PlankBridge.draw) PlankBridge.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PlankBridge !== 'undefined' && PlankBridge.tap) PlankBridge.tap(x,y);
...
// init
try {
  if (typeof PlankBridge !== 'undefined' && PlankBridge.init) PlankBridge.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (plank-bridge): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS plank-bridge');
}
