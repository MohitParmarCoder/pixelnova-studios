...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('ninjarun_best'), 10) || 0; } catch(e) {}
  NinjaRun.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); NinjaRun.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { NinjaRun.update(dt); }
    NinjaRun.draw();
    var b = NinjaRun.getBest ? NinjaRun.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('ninjarun_best', String(best)); } catch(e) {} }
