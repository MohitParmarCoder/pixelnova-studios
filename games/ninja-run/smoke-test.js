'use strict';
// Headless smoke test for ninja-run (NinjaRun)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof NinjaRun !== 'undefined' && NinjaRun.update) NinjaRun.update(dt);
    if (typeof NinjaRun !== 'undefined' && NinjaRun.draw) NinjaRun.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof NinjaRun !== 'undefined' && NinjaRun.tap) NinjaRun.tap(x,y);
...
// init
try {
  if (typeof NinjaRun !== 'undefined' && NinjaRun.init) NinjaRun.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (ninja-run): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS ninja-run');
}
