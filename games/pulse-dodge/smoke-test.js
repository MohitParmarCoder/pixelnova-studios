'use strict';
// Headless smoke test for pulse-dodge (PulseDodge)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PulseDodge !== 'undefined' && PulseDodge.update) PulseDodge.update(dt);
    if (typeof PulseDodge !== 'undefined' && PulseDodge.draw) PulseDodge.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PulseDodge !== 'undefined' && PulseDodge.tap) PulseDodge.tap(x,y);
...
// init
try {
  if (typeof PulseDodge !== 'undefined' && PulseDodge.init) PulseDodge.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (pulse-dodge): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS pulse-dodge');
}
