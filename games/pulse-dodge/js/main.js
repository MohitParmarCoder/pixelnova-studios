...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('pulsedodge_best'), 10) || 0; } catch(e) {}
  PulseDodge.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); PulseDodge.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { PulseDodge.update(dt); }
    PulseDodge.draw();
    var b = PulseDodge.getBest ? PulseDodge.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('pulsedodge_best', String(best)); } catch(e) {} }
