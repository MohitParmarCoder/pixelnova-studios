'use strict';
// Headless smoke test for balance-beam (BalanceBeam)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BalanceBeam !== 'undefined' && BalanceBeam.update) BalanceBeam.update(dt);
    if (typeof BalanceBeam !== 'undefined' && BalanceBeam.draw) BalanceBeam.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BalanceBeam !== 'undefined' && BalanceBeam.tap) BalanceBeam.tap(x,y);
...
// init
try {
  if (typeof BalanceBeam !== 'undefined' && BalanceBeam.init) BalanceBeam.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (balance-beam): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS balance-beam');
}
