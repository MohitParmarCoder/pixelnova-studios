...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('balancebeam_best'), 10) || 0; } catch(e) {}
  BalanceBeam.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); BalanceBeam.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { BalanceBeam.update(dt); }
    BalanceBeam.draw();
    var b = BalanceBeam.getBest ? BalanceBeam.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('balancebeam_best', String(best)); } catch(e) {} }
