'use strict';
// Headless smoke test for spike-field (SpikeField)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SpikeField !== 'undefined' && SpikeField.update) SpikeField.update(dt);
    if (typeof SpikeField !== 'undefined' && SpikeField.draw) SpikeField.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SpikeField !== 'undefined' && SpikeField.tap) SpikeField.tap(x,y);
...
// init
try {
  if (typeof SpikeField !== 'undefined' && SpikeField.init) SpikeField.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (spike-field): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS spike-field');
}
