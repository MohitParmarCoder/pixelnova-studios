...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('spacerunner_best'), 10) || 0; } catch(e) {}
  SpaceRunner.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); SpaceRunner.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { SpaceRunner.update(dt); }
    SpaceRunner.draw();
    var b = SpaceRunner.getBest ? SpaceRunner.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('spacerunner_best', String(best)); } catch(e) {} }
