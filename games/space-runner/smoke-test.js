'use strict';
// Headless smoke test for space-runner (SpaceRunner)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SpaceRunner !== 'undefined' && SpaceRunner.update) SpaceRunner.update(dt);
    if (typeof SpaceRunner !== 'undefined' && SpaceRunner.draw) SpaceRunner.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SpaceRunner !== 'undefined' && SpaceRunner.tap) SpaceRunner.tap(x,y);
...
// init
try {
  if (typeof SpaceRunner !== 'undefined' && SpaceRunner.init) SpaceRunner.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (space-runner): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS space-runner');
}
