'use strict';
// Headless smoke test for block-blast (BlockBlast)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BlockBlast !== 'undefined' && BlockBlast.update) BlockBlast.update(dt);
    if (typeof BlockBlast !== 'undefined' && BlockBlast.draw) BlockBlast.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BlockBlast !== 'undefined' && BlockBlast.tap) BlockBlast.tap(x,y);
...
// init
try {
  if (typeof BlockBlast !== 'undefined' && BlockBlast.init) BlockBlast.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (block-blast): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS block-blast');
}
