'use strict';
// Headless smoke test for symbol-hunt (SymbolHunt)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SymbolHunt !== 'undefined' && SymbolHunt.update) SymbolHunt.update(dt);
    if (typeof SymbolHunt !== 'undefined' && SymbolHunt.draw) SymbolHunt.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SymbolHunt !== 'undefined' && SymbolHunt.tap) SymbolHunt.tap(x,y);
...
// init
try {
  if (typeof SymbolHunt !== 'undefined' && SymbolHunt.init) SymbolHunt.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (symbol-hunt): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS symbol-hunt');
}
