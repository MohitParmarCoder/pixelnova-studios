'use strict';
// Headless smoke test for memory-flip (MemoryFlip)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof MemoryFlip !== 'undefined' && MemoryFlip.update) MemoryFlip.update(dt);
    if (typeof MemoryFlip !== 'undefined' && MemoryFlip.draw) MemoryFlip.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof MemoryFlip !== 'undefined' && MemoryFlip.tap) MemoryFlip.tap(x,y);
...
// init
try {
  if (typeof MemoryFlip !== 'undefined' && MemoryFlip.init) MemoryFlip.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (memory-flip): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS memory-flip');
}
