...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('eggdrop_best'), 10) || 0; } catch(e) {}
  EggDrop.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); EggDrop.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { EggDrop.update(dt); }
    EggDrop.draw();
    var b = EggDrop.getBest ? EggDrop.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('eggdrop_best', String(best)); } catch(e) {} }
