'use strict';
// Headless smoke test for egg-drop (EggDrop)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof EggDrop !== 'undefined' && EggDrop.update) EggDrop.update(dt);
    if (typeof EggDrop !== 'undefined' && EggDrop.draw) EggDrop.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof EggDrop !== 'undefined' && EggDrop.tap) EggDrop.tap(x,y);
...
// init
try {
  if (typeof EggDrop !== 'undefined' && EggDrop.init) EggDrop.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (egg-drop): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS egg-drop');
}
