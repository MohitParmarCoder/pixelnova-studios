'use strict';
// Headless smoke test for match-gems (MatchGems)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof MatchGems !== 'undefined' && MatchGems.update) MatchGems.update(dt);
    if (typeof MatchGems !== 'undefined' && MatchGems.draw) MatchGems.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof MatchGems !== 'undefined' && MatchGems.tap) MatchGems.tap(x,y);
...
// init
try {
  if (typeof MatchGems !== 'undefined' && MatchGems.init) MatchGems.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (match-gems): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS match-gems');
}
