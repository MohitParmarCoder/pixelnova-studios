...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('matchgems_best'), 10) || 0; } catch(e) {}
  MatchGems.init(canvas, best);
  ...
  function loop(ts) {
    ...
    if (!_paused) { MatchGems.update(dt); }
    MatchGems.draw();
    var b = MatchGems.getBest ? MatchGems.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('matchgems_best', String(best)); } catch(e) {} }
