...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('snowballcatch_best'), 10) || 0; } catch(e) {}
  SnowballCatch.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); SnowballCatch.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { SnowballCatch.update(dt); }
    SnowballCatch.draw();
    var b = SnowballCatch.getBest ? SnowballCatch.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('snowballcatch_best', String(best)); } catch(e) {} }
