'use strict';
// Headless smoke test for snowball-catch (SnowballCatch)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SnowballCatch !== 'undefined' && SnowballCatch.update) SnowballCatch.update(dt);
    if (typeof SnowballCatch !== 'undefined' && SnowballCatch.draw) SnowballCatch.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SnowballCatch !== 'undefined' && SnowballCatch.tap) SnowballCatch.tap(x,y);
...
// init
try {
  if (typeof SnowballCatch !== 'undefined' && SnowballCatch.init) SnowballCatch.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (snowball-catch): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS snowball-catch');
}
