...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('trafficrush_best'), 10) || 0; } catch(e) {}
  TrafficRush.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); TrafficRush.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { TrafficRush.update(dt); }
    TrafficRush.draw();
    var b = TrafficRush.getBest ? TrafficRush.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('trafficrush_best', String(best)); } catch(e) {} }
