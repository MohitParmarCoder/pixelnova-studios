'use strict';
// Headless smoke test for traffic-rush (TrafficRush)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof TrafficRush !== 'undefined' && TrafficRush.update) TrafficRush.update(dt);
    if (typeof TrafficRush !== 'undefined' && TrafficRush.draw) TrafficRush.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof TrafficRush !== 'undefined' && TrafficRush.tap) TrafficRush.tap(x,y);
...
// init
try {
  if (typeof TrafficRush !== 'undefined' && TrafficRush.init) TrafficRush.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (traffic-rush): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS traffic-rush');
}
