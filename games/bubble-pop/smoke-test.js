'use strict';
// Headless smoke test for bubble-pop (BubblePop)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BubblePop !== 'undefined' && BubblePop.update) BubblePop.update(dt);
    if (typeof BubblePop !== 'undefined' && BubblePop.draw) BubblePop.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BubblePop !== 'undefined' && BubblePop.tap) BubblePop.tap(x,y);
...
// init
try {
  if (typeof BubblePop !== 'undefined' && BubblePop.init) BubblePop.init(canvasStub, 0);
} catch(e) { errors.push('init: '+e.message.slice(0,80)); }

// idle
frames(60);
// start
tap(195, 800); frames(120);
...
if (uniq.length) {
  console.error('FAIL (bubble-pop): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS bubble-pop');
}
