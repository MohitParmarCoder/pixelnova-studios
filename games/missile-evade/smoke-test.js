'use strict';
// Headless smoke test for missile-evade (MissileEvade)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof MissileEvade !== 'undefined' && MissileEvade.update) MissileEvade.update(dt);
    if (typeof MissileEvade !== 'undefined' && MissileEvade.draw) MissileEvade.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof MissileEvade !== 'undefined' && MissileEvade.tap) MissileEvade.tap(x,y);
...
// init
try {
  if (typeof MissileEvade !== 'undefined' && MissileEvade.init) MissileEvade.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (missile-evade): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS missile-evade');
}
