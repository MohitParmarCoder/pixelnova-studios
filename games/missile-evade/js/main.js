...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('missileevade_best'), 10) || 0; } catch(e) {}
  MissileEvade.init(canvas, best);
  ...
  canvas.addEventListener('pointerdown', function (e) {
    e.preventDefault();
    var p = toVirtual(e);
    MissileEvade.tap(p.x, p.y);
  });
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); MissileEvade.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { MissileEvade.update(dt); }
    MissileEvade.draw();
    var b = MissileEvade.getBest ? MissileEvade.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('missileevade_best', String(best)); } catch(e) {} }
