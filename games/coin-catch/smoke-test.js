'use strict';
// Headless smoke test for coin-catch (CoinCatch)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof CoinCatch !== 'undefined' && CoinCatch.update) CoinCatch.update(dt);
    if (typeof CoinCatch !== 'undefined' && CoinCatch.draw) CoinCatch.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof CoinCatch !== 'undefined' && CoinCatch.tap) CoinCatch.tap(x,y);
...
// init
try {
  if (typeof CoinCatch !== 'undefined' && CoinCatch.init) CoinCatch.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (coin-catch): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS coin-catch');
}
