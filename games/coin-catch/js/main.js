...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('coincatch_best'), 10) || 0; } catch(e) {}
  CoinCatch.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); CoinCatch.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { CoinCatch.update(dt); }
    CoinCatch.draw();
    var b = CoinCatch.getBest ? CoinCatch.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('coincatch_best', String(best)); } catch(e) {} }
