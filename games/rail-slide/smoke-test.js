'use strict';
// Headless smoke test for rail-slide (RailSlide)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof RailSlide !== 'undefined' && RailSlide.update) RailSlide.update(dt);
    if (typeof RailSlide !== 'undefined' && RailSlide.draw) RailSlide.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof RailSlide !== 'undefined' && RailSlide.tap) RailSlide.tap(x,y);
...
// init
try {
  if (typeof RailSlide !== 'undefined' && RailSlide.init) RailSlide.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (rail-slide): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS rail-slide');
}
