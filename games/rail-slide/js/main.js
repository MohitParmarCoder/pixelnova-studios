...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('railslide_best'), 10) || 0; } catch(e) {}
  RailSlide.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); RailSlide.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { RailSlide.update(dt); }
    RailSlide.draw();
    var b = RailSlide.getBest ? RailSlide.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('railslide_best', String(best)); } catch(e) {} }
