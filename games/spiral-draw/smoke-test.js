'use strict';
// Headless smoke test for spiral-draw (SpiralDraw)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SpiralDraw !== 'undefined' && SpiralDraw.update) SpiralDraw.update(dt);
    if (typeof SpiralDraw !== 'undefined' && SpiralDraw.draw) SpiralDraw.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SpiralDraw !== 'undefined' && SpiralDraw.tap) SpiralDraw.tap(x,y);
...
// init
try {
  if (typeof SpiralDraw !== 'undefined' && SpiralDraw.init) SpiralDraw.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (spiral-draw): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS spiral-draw');
}
