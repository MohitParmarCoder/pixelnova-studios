...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('spiraldraw_best'), 10) || 0; } catch(e) {}
  SpiralDraw.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); SpiralDraw.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { SpiralDraw.update(dt); }
    SpiralDraw.draw();
    var b = SpiralDraw.getBest ? SpiralDraw.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('spiraldraw_best', String(best)); } catch(e) {} }
