...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('pinballlite_best'), 10) || 0; } catch(e) {}
  PinballLite.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); PinballLite.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { PinballLite.update(dt); }
    PinballLite.draw();
    var b = PinballLite.getBest ? PinballLite.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('pinballlite_best', String(best)); } catch(e) {} }
