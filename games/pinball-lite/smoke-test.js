'use strict';
// Headless smoke test for pinball-lite (PinballLite)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PinballLite !== 'undefined' && PinballLite.update) PinballLite.update(dt);
    if (typeof PinballLite !== 'undefined' && PinballLite.draw) PinballLite.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PinballLite !== 'undefined' && PinballLite.tap) PinballLite.tap(x,y);
...
// init
try {
  if (typeof PinballLite !== 'undefined' && PinballLite.init) PinballLite.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (pinball-lite): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS pinball-lite');
}
