...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('billiardaim_best'), 10) || 0; } catch(e) {}
  BilliardAim.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); BilliardAim.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { BilliardAim.update(dt); }
    BilliardAim.draw();
    var b = BilliardAim.getBest ? BilliardAim.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('billiardaim_best', String(best)); } catch(e) {} }
