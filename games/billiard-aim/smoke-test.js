'use strict';
// Headless smoke test for billiard-aim (BilliardAim)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BilliardAim !== 'undefined' && BilliardAim.update) BilliardAim.update(dt);
    if (typeof BilliardAim !== 'undefined' && BilliardAim.draw) BilliardAim.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BilliardAim !== 'undefined' && BilliardAim.tap) BilliardAim.tap(x,y);
...
// init
try {
  if (typeof BilliardAim !== 'undefined' && BilliardAim.init) BilliardAim.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (billiard-aim): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS billiard-aim');
}
