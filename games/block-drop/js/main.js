...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('blockdrop_best'), 10) || 0; } catch(e) {}
  BlockDrop.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); BlockDrop.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { BlockDrop.update(dt); }
    BlockDrop.draw();
    var b = BlockDrop.getBest ? BlockDrop.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('blockdrop_best', String(best)); } catch(e) {} }
