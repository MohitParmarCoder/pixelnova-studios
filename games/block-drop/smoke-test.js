'use strict';
// Headless smoke test for block-drop (BlockDrop)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BlockDrop !== 'undefined' && BlockDrop.update) BlockDrop.update(dt);
    if (typeof BlockDrop !== 'undefined' && BlockDrop.draw) BlockDrop.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BlockDrop !== 'undefined' && BlockDrop.tap) BlockDrop.tap(x,y);
...
// init
try {
  if (typeof BlockDrop !== 'undefined' && BlockDrop.init) BlockDrop.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (block-drop): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS block-drop');
}
