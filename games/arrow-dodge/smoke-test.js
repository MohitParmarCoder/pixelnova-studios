'use strict';
// Headless smoke test for arrow-dodge (ArrowDodge)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ArrowDodge !== 'undefined' && ArrowDodge.update) ArrowDodge.update(dt);
    if (typeof ArrowDodge !== 'undefined' && ArrowDodge.draw) ArrowDodge.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ArrowDodge !== 'undefined' && ArrowDodge.tap) ArrowDodge.tap(x,y);
...
// init
try {
  if (typeof ArrowDodge !== 'undefined' && ArrowDodge.init) ArrowDodge.init(canvasStub, 0);
} catch(e) { errors.push('init: '+e.message.slice(0,80)); }

// idle
frames(60);
// start
tap(195, 760); frames(120);
...
if (uniq.length) {
  console.error('FAIL (arrow-dodge): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS arrow-dodge');
}
