...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('arrowdodge_best'), 10) || 0; } catch(e) {}
  ArrowDodge.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ArrowDodge.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ArrowDodge.update(dt); }
    ArrowDodge.draw();
    var b = ArrowDodge.getBest ? ArrowDodge.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('arrowdodge_best', String(best)); } catch(e) {} }
