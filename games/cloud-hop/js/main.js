...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('cloudhop_best'), 10) || 0; } catch(e) {}
  CloudHop.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); CloudHop.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { CloudHop.update(dt); }
    CloudHop.draw();
    var b = CloudHop.getBest ? CloudHop.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('cloudhop_best', String(best)); } catch(e) {} }
