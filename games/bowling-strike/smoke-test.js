'use strict';
// Headless smoke test for bowling-strike (BowlingStrike)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BowlingStrike !== 'undefined' && BowlingStrike.update) BowlingStrike.update(dt);
    if (typeof BowlingStrike !== 'undefined' && BowlingStrike.draw) BowlingStrike.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BowlingStrike !== 'undefined' && BowlingStrike.tap) BowlingStrike.tap(x,y);
...
// init
try {
  if (typeof BowlingStrike !== 'undefined' && BowlingStrike.init) BowlingStrike.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (bowling-strike): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS bowling-strike');
}
