...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('bowlingstrike_best'), 10) || 0; } catch(e) {}
  BowlingStrike.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); BowlingStrike.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { BowlingStrike.update(dt); }
    BowlingStrike.draw();
    var b = BowlingStrike.getBest ? BowlingStrike.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('bowlingstrike_best', String(best)); } catch(e) {} }
