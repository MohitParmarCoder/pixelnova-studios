'use strict';
// Headless smoke test for asteroid-belt (AsteroidBelt)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof AsteroidBelt !== 'undefined' && AsteroidBelt.update) AsteroidBelt.update(dt);
    if (typeof AsteroidBelt !== 'undefined' && AsteroidBelt.draw) AsteroidBelt.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof AsteroidBelt !== 'undefined' && AsteroidBelt.tap) AsteroidBelt.tap(x,y);
...
// init
try {
  if (typeof AsteroidBelt !== 'undefined' && AsteroidBelt.init) AsteroidBelt.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (asteroid-belt): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS asteroid-belt');
}
