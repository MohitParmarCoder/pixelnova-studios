...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('asteroidbelt_best'), 10) || 0; } catch(e) {}
  AsteroidBelt.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); AsteroidBelt.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { AsteroidBelt.update(dt); }
    AsteroidBelt.draw();
    var b = AsteroidBelt.getBest ? AsteroidBelt.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('asteroidbelt_best', String(best)); } catch(e) {} }
