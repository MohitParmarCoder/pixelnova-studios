'use strict';
// Headless smoke test for crystal-stack (CrystalStack)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof CrystalStack !== 'undefined' && CrystalStack.update) CrystalStack.update(dt);
    if (typeof CrystalStack !== 'undefined' && CrystalStack.draw) CrystalStack.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof CrystalStack !== 'undefined' && CrystalStack.tap) CrystalStack.tap(x,y);
...
// init
try {
  if (typeof CrystalStack !== 'undefined' && CrystalStack.init) CrystalStack.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (crystal-stack): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS crystal-stack');
}
