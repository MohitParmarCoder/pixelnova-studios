...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('crystalstack_best'), 10) || 0; } catch(e) {}
  CrystalStack.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); CrystalStack.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { CrystalStack.update(dt); }
    CrystalStack.draw();
    var b = CrystalStack.getBest ? CrystalStack.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('crystalstack_best', String(best)); } catch(e) {} }
