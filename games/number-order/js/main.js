...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('numberorder_best'), 10) || 0; } catch(e) {}
  NumberOrder.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); NumberOrder.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { NumberOrder.update(dt); }
    NumberOrder.draw();
    var b = NumberOrder.getBest ? NumberOrder.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('numberorder_best', String(best)); } catch(e) {} }
