'use strict';
// Headless smoke test for number-order (NumberOrder)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof NumberOrder !== 'undefined' && NumberOrder.update) NumberOrder.update(dt);
    if (typeof NumberOrder !== 'undefined' && NumberOrder.draw) NumberOrder.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof NumberOrder !== 'undefined' && NumberOrder.tap) NumberOrder.tap(x,y);
...
// init
try {
  if (typeof NumberOrder !== 'undefined' && NumberOrder.init) NumberOrder.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (number-order): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS number-order');
}
