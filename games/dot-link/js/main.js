...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('dotlink_best'), 10) || 0; } catch(e) {}
  DotLink.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); DotLink.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { DotLink.update(dt); }
    DotLink.draw();
    var b = DotLink.getBest ? DotLink.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('dotlink_best', String(best)); } catch(e) {} }
