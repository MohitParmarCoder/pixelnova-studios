'use strict';
// Headless smoke test for dot-link (DotLink)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof DotLink !== 'undefined' && DotLink.update) DotLink.update(dt);
    if (typeof DotLink !== 'undefined' && DotLink.draw) DotLink.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof DotLink !== 'undefined' && DotLink.tap) DotLink.tap(x,y);
...
// init
try {
  if (typeof DotLink !== 'undefined' && DotLink.init) DotLink.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (dot-link): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS dot-link');
}
