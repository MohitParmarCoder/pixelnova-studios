'use strict';
// Headless smoke test for dodge-rush (DodgeRush)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof DodgeRush !== 'undefined' && DodgeRush.update) DodgeRush.update(dt);
    if (typeof DodgeRush !== 'undefined' && DodgeRush.draw) DodgeRush.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof DodgeRush !== 'undefined' && DodgeRush.tap) DodgeRush.tap(x,y);
...
// init
try {
  if (typeof DodgeRush !== 'undefined' && DodgeRush.init) DodgeRush.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (dodge-rush): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS dodge-rush');
}
