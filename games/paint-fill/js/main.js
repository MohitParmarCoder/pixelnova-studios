...
(function () {
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); PaintFill.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { PaintFill.update(dt); }
    PaintFill.draw();
    var b = PaintFill.getBest ? PaintFill.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('paintfill_best', String(best)); } catch(e) {} }
