'use strict';
// Headless smoke test for paint-fill (PaintFill)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PaintFill !== 'undefined' && PaintFill.update) PaintFill.update(dt);
    if (typeof PaintFill !== 'undefined' && PaintFill.draw) PaintFill.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PaintFill !== 'undefined' && PaintFill.tap) PaintFill.tap(x,y);
...
// init
try {
  if (typeof PaintFill !== 'undefined' && PaintFill.init) PaintFill.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (paint-fill): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS paint-fill');
}
