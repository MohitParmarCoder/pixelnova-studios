...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('wordblitz_best'), 10) || 0; } catch(e) {}
  WordBlitz.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); WordBlitz.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { WordBlitz.update(dt); }
    WordBlitz.draw();
    var b = WordBlitz.getBest ? WordBlitz.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('wordblitz_best', String(best)); } catch(e) {} }
