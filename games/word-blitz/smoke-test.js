'use strict';
// Headless smoke test for word-blitz (WordBlitz)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof WordBlitz !== 'undefined' && WordBlitz.update) WordBlitz.update(dt);
    if (typeof WordBlitz !== 'undefined' && WordBlitz.draw) WordBlitz.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof WordBlitz !== 'undefined' && WordBlitz.tap) WordBlitz.tap(x,y);
...
// init
try {
  if (typeof WordBlitz !== 'undefined' && WordBlitz.init) WordBlitz.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (word-blitz): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS word-blitz');
}
