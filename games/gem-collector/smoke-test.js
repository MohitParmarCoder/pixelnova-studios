'use strict';
// Headless smoke test for gem-collector (GemCollector)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof GemCollector !== 'undefined' && GemCollector.update) GemCollector.update(dt);
    if (typeof GemCollector !== 'undefined' && GemCollector.draw) GemCollector.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof GemCollector !== 'undefined' && GemCollector.tap) GemCollector.tap(x,y);
...
// init
try {
  if (typeof GemCollector !== 'undefined' && GemCollector.init) GemCollector.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (gem-collector): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS gem-collector');
}
