...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('gemcollector_best'), 10) || 0; } catch(e) {}
  GemCollector.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); GemCollector.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { GemCollector.update(dt); }
    GemCollector.draw();
    var b = GemCollector.getBest ? GemCollector.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('gemcollector_best', String(best)); } catch(e) {} }
