'use strict';
// Headless smoke test for neon-path (NeonPath)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof NeonPath !== 'undefined' && NeonPath.update) NeonPath.update(dt);
    if (typeof NeonPath !== 'undefined' && NeonPath.draw) NeonPath.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof NeonPath !== 'undefined' && NeonPath.tap) NeonPath.tap(x,y);
...
// init
try {
  if (typeof NeonPath !== 'undefined' && NeonPath.init) NeonPath.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (neon-path): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS neon-path');
}
