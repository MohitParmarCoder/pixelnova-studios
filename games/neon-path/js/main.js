...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('neonpath_best'), 10) || 0; } catch(e) {}
  NeonPath.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); NeonPath.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { NeonPath.update(dt); }
    NeonPath.draw();
    var b = NeonPath.getBest ? NeonPath.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('neonpath_best', String(best)); } catch(e) {} }
