'use strict';
// Headless smoke test for color-burst (ColorBurst)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ColorBurst !== 'undefined' && ColorBurst.update) ColorBurst.update(dt);
    if (typeof ColorBurst !== 'undefined' && ColorBurst.draw) ColorBurst.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ColorBurst !== 'undefined' && ColorBurst.tap) ColorBurst.tap(x,y);
...
// init
try {
  if (typeof ColorBurst !== 'undefined' && ColorBurst.init) ColorBurst.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (color-burst): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS color-burst');
}
