...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('colorburst_best'), 10) || 0; } catch(e) {}
  ColorBurst.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ColorBurst.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ColorBurst.update(dt); }
    ColorBurst.draw();
    var b = ColorBurst.getBest ? ColorBurst.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('colorburst_best', String(best)); } catch(e) {} }
