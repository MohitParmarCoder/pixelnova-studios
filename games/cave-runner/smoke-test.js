'use strict';
// Headless smoke test for cave-runner (CaveRunner)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof CaveRunner !== 'undefined' && CaveRunner.update) CaveRunner.update(dt);
    if (typeof CaveRunner !== 'undefined' && CaveRunner.draw) CaveRunner.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof CaveRunner !== 'undefined' && CaveRunner.tap) CaveRunner.tap(x,y);
...
// init
try {
  if (typeof CaveRunner !== 'undefined' && CaveRunner.init) CaveRunner.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (cave-runner): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS cave-runner');
}
