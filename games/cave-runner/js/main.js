...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('caverunner_best'), 10) || 0; } catch(e) {}
  CaveRunner.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); CaveRunner.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { CaveRunner.update(dt); }
    CaveRunner.draw();
    var b = CaveRunner.getBest ? CaveRunner.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('caverunner_best', String(best)); } catch(e) {} }
