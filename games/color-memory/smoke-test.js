'use strict';
// Headless smoke test for color-memory (ColorMemory)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ColorMemory !== 'undefined' && ColorMemory.update) ColorMemory.update(dt);
    if (typeof ColorMemory !== 'undefined' && ColorMemory.draw) ColorMemory.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ColorMemory !== 'undefined' && ColorMemory.tap) ColorMemory.tap(x,y);
...
// init
try {
  if (typeof ColorMemory !== 'undefined' && ColorMemory.init) ColorMemory.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (color-memory): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS color-memory');
}
