'use strict';
// Headless smoke test for mirror-tap (MirrorTap)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof MirrorTap !== 'undefined' && MirrorTap.update) MirrorTap.update(dt);
    if (typeof MirrorTap !== 'undefined' && MirrorTap.draw) MirrorTap.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof MirrorTap !== 'undefined' && MirrorTap.tap) MirrorTap.tap(x,y);
...
// init
try {
  if (typeof MirrorTap !== 'undefined' && MirrorTap.init) MirrorTap.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (mirror-tap): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS mirror-tap');
}
