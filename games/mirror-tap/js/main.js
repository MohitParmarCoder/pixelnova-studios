...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('mirrortap_best'), 10) || 0; } catch(e) {}
  MirrorTap.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); MirrorTap.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { MirrorTap.update(dt); }
    MirrorTap.draw();
    var b = MirrorTap.getBest ? MirrorTap.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('mirrortap_best', String(best)); } catch(e) {} }
