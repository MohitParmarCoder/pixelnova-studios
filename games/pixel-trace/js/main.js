...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('pixeltrace_best'), 10) || 0; } catch(e) {}
  PixelTrace.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); PixelTrace.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { PixelTrace.update(dt); }
    PixelTrace.draw();
    var b = PixelTrace.getBest ? PixelTrace.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('pixeltrace_best', String(best)); } catch(e) {} }
