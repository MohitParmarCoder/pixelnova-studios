'use strict';
// Headless smoke test for pixel-trace (PixelTrace)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PixelTrace !== 'undefined' && PixelTrace.update) PixelTrace.update(dt);
    if (typeof PixelTrace !== 'undefined' && PixelTrace.draw) PixelTrace.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PixelTrace !== 'undefined' && PixelTrace.tap) PixelTrace.tap(x,y);
...
// init
try {
  if (typeof PixelTrace !== 'undefined' && PixelTrace.init) PixelTrace.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (pixel-trace): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS pixel-trace');
}
