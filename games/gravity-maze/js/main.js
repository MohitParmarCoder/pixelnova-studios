...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('gravitymaze_best'), 10) || 0; } catch(e) {}
  GravityMaze.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); GravityMaze.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { GravityMaze.update(dt); }
    GravityMaze.draw();
    var b = GravityMaze.getBest ? GravityMaze.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('gravitymaze_best', String(best)); } catch(e) {} }
