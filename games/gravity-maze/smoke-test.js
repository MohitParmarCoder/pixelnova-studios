'use strict';
// Headless smoke test for gravity-maze (GravityMaze)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof GravityMaze !== 'undefined' && GravityMaze.update) GravityMaze.update(dt);
    if (typeof GravityMaze !== 'undefined' && GravityMaze.draw) GravityMaze.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof GravityMaze !== 'undefined' && GravityMaze.tap) GravityMaze.tap(x,y);
...
// init
try {
  if (typeof GravityMaze !== 'undefined' && GravityMaze.init) GravityMaze.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (gravity-maze): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS gravity-maze');
}
