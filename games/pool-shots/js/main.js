...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('poolshots_best'), 10) || 0; } catch(e) {}
  PoolShots.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); PoolShots.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { PoolShots.update(dt); }
    PoolShots.draw();
    var b = PoolShots.getBest ? PoolShots.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('poolshots_best', String(best)); } catch(e) {} }
