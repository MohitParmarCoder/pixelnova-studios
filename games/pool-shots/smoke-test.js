'use strict';
// Headless smoke test for pool-shots (PoolShots)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PoolShots !== 'undefined' && PoolShots.update) PoolShots.update(dt);
    if (typeof PoolShots !== 'undefined' && PoolShots.draw) PoolShots.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PoolShots !== 'undefined' && PoolShots.tap) PoolShots.tap(x,y);
...
// init
try {
  if (typeof PoolShots !== 'undefined' && PoolShots.init) PoolShots.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (pool-shots): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS pool-shots');
}
