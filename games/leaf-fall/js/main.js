...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('leaffall_best'), 10) || 0; } catch(e) {}
  LeafFall.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); LeafFall.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { LeafFall.update(dt); }
    LeafFall.draw();
    var b = LeafFall.getBest ? LeafFall.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('leaffall_best', String(best)); } catch(e) {} }
