'use strict';
// Headless smoke test for leaf-fall (LeafFall)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof LeafFall !== 'undefined' && LeafFall.update) LeafFall.update(dt);
    if (typeof LeafFall !== 'undefined' && LeafFall.draw) LeafFall.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof LeafFall !== 'undefined' && LeafFall.tap) LeafFall.tap(x,y);
...
// init
try {
  if (typeof LeafFall !== 'undefined' && LeafFall.init) LeafFall.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (leaf-fall): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS leaf-fall');
}
