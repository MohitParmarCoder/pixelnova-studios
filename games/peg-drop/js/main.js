...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('pegdrop_best'), 10) || 0; } catch(e) {}
  PegDrop.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); PegDrop.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { PegDrop.update(dt); }
    PegDrop.draw();
    var b = PegDrop.getBest ? PegDrop.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('pegdrop_best', String(best)); } catch(e) {} }
