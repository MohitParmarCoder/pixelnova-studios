'use strict';
// Headless smoke test for peg-drop (PegDrop)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PegDrop !== 'undefined' && PegDrop.update) PegDrop.update(dt);
    if (typeof PegDrop !== 'undefined' && PegDrop.draw) PegDrop.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PegDrop !== 'undefined' && PegDrop.tap) PegDrop.tap(x,y);
...
// init
try {
  if (typeof PegDrop !== 'undefined' && PegDrop.init) PegDrop.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (peg-drop): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS peg-drop');
}
