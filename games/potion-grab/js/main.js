...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('potiongrab_best'), 10) || 0; } catch(e) {}
  PotionGrab.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); PotionGrab.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { PotionGrab.update(dt); }
    PotionGrab.draw();
    var b = PotionGrab.getBest ? PotionGrab.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('potiongrab_best', String(best)); } catch(e) {} }
