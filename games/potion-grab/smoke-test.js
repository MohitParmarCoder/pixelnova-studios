'use strict';
// Headless smoke test for potion-grab (PotionGrab)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PotionGrab !== 'undefined' && PotionGrab.update) PotionGrab.update(dt);
    if (typeof PotionGrab !== 'undefined' && PotionGrab.draw) PotionGrab.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PotionGrab !== 'undefined' && PotionGrab.tap) PotionGrab.tap(x,y);
...
// init
try {
  if (typeof PotionGrab !== 'undefined' && PotionGrab.init) PotionGrab.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (potion-grab): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS potion-grab');
}
