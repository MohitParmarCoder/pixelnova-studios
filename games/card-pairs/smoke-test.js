'use strict';
// Headless smoke test for card-pairs (CardPairs)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof CardPairs !== 'undefined' && CardPairs.update) CardPairs.update(dt);
    if (typeof CardPairs !== 'undefined' && CardPairs.draw) CardPairs.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof CardPairs !== 'undefined' && CardPairs.tap) CardPairs.tap(x,y);
...
// init
try {
  if (typeof CardPairs !== 'undefined' && CardPairs.init) CardPairs.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (card-pairs): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS card-pairs');
}
