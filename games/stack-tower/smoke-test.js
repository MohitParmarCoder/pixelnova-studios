'use strict';
// Headless smoke test for stack-tower (StackTower)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof StackTower !== 'undefined' && StackTower.update) StackTower.update(dt);
    if (typeof StackTower !== 'undefined' && StackTower.draw) StackTower.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof StackTower !== 'undefined' && StackTower.tap) StackTower.tap(x,y);
...
// init
try {
  if (typeof StackTower !== 'undefined' && StackTower.init) StackTower.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (stack-tower): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS stack-tower');
}
