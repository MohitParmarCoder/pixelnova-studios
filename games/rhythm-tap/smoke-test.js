'use strict';
// Headless smoke test for rhythm-tap (RhythmTap)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof RhythmTap !== 'undefined' && RhythmTap.update) RhythmTap.update(dt);
    if (typeof RhythmTap !== 'undefined' && RhythmTap.draw) RhythmTap.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof RhythmTap !== 'undefined' && RhythmTap.tap) RhythmTap.tap(x,y);
...
// init
try {
  if (typeof RhythmTap !== 'undefined' && RhythmTap.init) RhythmTap.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (rhythm-tap): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS rhythm-tap');
}
