...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('rhythmtap_best'), 10) || 0; } catch(e) {}
  RhythmTap.init(canvas, best);
  ...
  canvas.addEventListener('pointerdown', function (e) {
    e.preventDefault();
    var p = toVirtual(e);
    RhythmTap.tap(p.x, p.y);
  });
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); RhythmTap.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { RhythmTap.update(dt); }
    RhythmTap.draw();
    var b = RhythmTap.getBest ? RhythmTap.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('rhythmtap_best', String(best)); } catch(e) {} }
