'use strict';
// Headless smoke test for zombie-smash (ZombieSmash)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ZombieSmash !== 'undefined' && ZombieSmash.update) ZombieSmash.update(dt);
    if (typeof ZombieSmash !== 'undefined' && ZombieSmash.draw) ZombieSmash.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ZombieSmash !== 'undefined' && ZombieSmash.tap) ZombieSmash.tap(x,y);
...
// init
try {
  if (typeof ZombieSmash !== 'undefined' && ZombieSmash.init) ZombieSmash.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (zombie-smash): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS zombie-smash');
}
