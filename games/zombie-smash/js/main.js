...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('zombiesmash_best'), 10) || 0; } catch(e) {}
  ZombieSmash.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ZombieSmash.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ZombieSmash.update(dt); }
    ZombieSmash.draw();
    var b = ZombieSmash.getBest ? ZombieSmash.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('zombiesmash_best', String(best)); } catch(e) {} }
