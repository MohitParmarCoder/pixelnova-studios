...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('endlessrunner_best'), 10) || 0; } catch(e) {}
  EndlessRunner.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); EndlessRunner.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { EndlessRunner.update(dt); }
    EndlessRunner.draw();
    var b = EndlessRunner.getBest ? EndlessRunner.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('endlessrunner_best', String(best)); } catch(e) {} }
