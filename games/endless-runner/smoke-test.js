'use strict';
// Headless smoke test for endless-runner (EndlessRunner)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof EndlessRunner !== 'undefined' && EndlessRunner.update) EndlessRunner.update(dt);
    if (typeof EndlessRunner !== 'undefined' && EndlessRunner.draw) EndlessRunner.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof EndlessRunner !== 'undefined' && EndlessRunner.tap) EndlessRunner.tap(x,y);
...
// init
try {
  if (typeof EndlessRunner !== 'undefined' && EndlessRunner.init) EndlessRunner.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (endless-runner): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS endless-runner');
}
