'use strict';
// Headless smoke test for wall-rise (WallRise)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof WallRise !== 'undefined' && WallRise.update) WallRise.update(dt);
    if (typeof WallRise !== 'undefined' && WallRise.draw) WallRise.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof WallRise !== 'undefined' && WallRise.tap) WallRise.tap(x,y);
...
// init
try {
  if (typeof WallRise !== 'undefined' && WallRise.init) WallRise.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (wall-rise): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS wall-rise');
}
