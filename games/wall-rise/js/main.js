...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('wallrise_best'), 10) || 0; } catch(e) {}
  WallRise.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); WallRise.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { WallRise.update(dt); }
    WallRise.draw();
    var b = WallRise.getBest ? WallRise.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('wallrise_best', String(best)); } catch(e) {} }
