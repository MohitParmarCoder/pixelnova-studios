...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('shadowslide_best'), 10) || 0; } catch(e) {}
  ShadowSlide.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ShadowSlide.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ShadowSlide.update(dt); }
    ShadowSlide.draw();
    var b = ShadowSlide.getBest ? ShadowSlide.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('shadowslide_best', String(best)); } catch(e) {} }
