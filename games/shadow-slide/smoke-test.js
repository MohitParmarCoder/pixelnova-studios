'use strict';
// Headless smoke test for shadow-slide (ShadowSlide)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ShadowSlide !== 'undefined' && ShadowSlide.update) ShadowSlide.update(dt);
    if (typeof ShadowSlide !== 'undefined' && ShadowSlide.draw) ShadowSlide.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ShadowSlide !== 'undefined' && ShadowSlide.tap) ShadowSlide.tap(x,y);
...
// init
try {
  if (typeof ShadowSlide !== 'undefined' && ShadowSlide.init) ShadowSlide.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (shadow-slide): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS shadow-slide');
}
