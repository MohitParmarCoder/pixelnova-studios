'use strict';
// Headless smoke test for basket-shot (BasketShot)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BasketShot !== 'undefined' && BasketShot.update) BasketShot.update(dt);
    if (typeof BasketShot !== 'undefined' && BasketShot.draw) BasketShot.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BasketShot !== 'undefined' && BasketShot.tap) BasketShot.tap(x,y);
...
// init
try {
  if (typeof BasketShot !== 'undefined' && BasketShot.init) BasketShot.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (basket-shot): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS basket-shot');
}
