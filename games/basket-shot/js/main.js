...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('basketshot_best'), 10) || 0; } catch(e) {}
  BasketShot.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); BasketShot.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { BasketShot.update(dt); }
    BasketShot.draw();
    var b = BasketShot.getBest ? BasketShot.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('basketshot_best', String(best)); } catch(e) {} }
