'use strict';
// Headless smoke test for sequence-game (SequenceGame)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SequenceGame !== 'undefined' && SequenceGame.update) SequenceGame.update(dt);
    if (typeof SequenceGame !== 'undefined' && SequenceGame.draw) SequenceGame.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SequenceGame !== 'undefined' && SequenceGame.tap) SequenceGame.tap(x,y);
...
// init
try {
  if (typeof SequenceGame !== 'undefined' && SequenceGame.init) SequenceGame.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (sequence-game): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS sequence-game');
}
