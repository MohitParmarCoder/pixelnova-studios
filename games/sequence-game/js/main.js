...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('sequencegame_best'), 10) || 0; } catch(e) {}
  SequenceGame.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); SequenceGame.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { SequenceGame.update(dt); }
    SequenceGame.draw();
    var b = SequenceGame.getBest ? SequenceGame.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('sequencegame_best', String(best)); } catch(e) {} }
