...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('orbcollector_best'), 10) || 0; } catch(e) {}
  OrbCollector.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); OrbCollector.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { OrbCollector.update(dt); }
    OrbCollector.draw();
    var b = OrbCollector.getBest ? OrbCollector.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('orbcollector_best', String(best)); } catch(e) {} }
