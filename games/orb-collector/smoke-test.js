'use strict';
// Headless smoke test for orb-collector (OrbCollector)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof OrbCollector !== 'undefined' && OrbCollector.update) OrbCollector.update(dt);
    if (typeof OrbCollector !== 'undefined' && OrbCollector.draw) OrbCollector.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof OrbCollector !== 'undefined' && OrbCollector.tap) OrbCollector.tap(x,y);
...
// init
try {
  if (typeof OrbCollector !== 'undefined' && OrbCollector.init) OrbCollector.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (orb-collector): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS orb-collector');
}
