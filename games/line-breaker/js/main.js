...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('linebreaker_best'), 10) || 0; } catch(e) {}
  LineBreaker.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); LineBreaker.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { LineBreaker.update(dt); }
    LineBreaker.draw();
    var b = LineBreaker.getBest ? LineBreaker.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('linebreaker_best', String(best)); } catch(e) {} }
