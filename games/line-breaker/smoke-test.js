'use strict';
// Headless smoke test for line-breaker (LineBreaker)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof LineBreaker !== 'undefined' && LineBreaker.update) LineBreaker.update(dt);
    if (typeof LineBreaker !== 'undefined' && LineBreaker.draw) LineBreaker.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof LineBreaker !== 'undefined' && LineBreaker.tap) LineBreaker.tap(x,y);
...
// init
try {
  if (typeof LineBreaker !== 'undefined' && LineBreaker.init) LineBreaker.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (line-breaker): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS line-breaker');
}
