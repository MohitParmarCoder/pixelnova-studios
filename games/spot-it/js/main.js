...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('spotit_best'), 10) || 0; } catch(e) {}
  SpotIt.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); SpotIt.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { SpotIt.update(dt); }
    SpotIt.draw();
    var b = SpotIt.getBest ? SpotIt.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('spotit_best', String(best)); } catch(e) {} }
