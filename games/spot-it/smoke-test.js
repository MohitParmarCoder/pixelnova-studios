'use strict';
// Headless smoke test for spot-it (SpotIt)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SpotIt !== 'undefined' && SpotIt.update) SpotIt.update(dt);
    if (typeof SpotIt !== 'undefined' && SpotIt.draw) SpotIt.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SpotIt !== 'undefined' && SpotIt.tap) SpotIt.tap(x,y);
...
// init
try {
  if (typeof SpotIt !== 'undefined' && SpotIt.init) SpotIt.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (spot-it): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS spot-it');
}
