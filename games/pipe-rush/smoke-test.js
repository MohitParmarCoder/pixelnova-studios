'use strict';
// Headless smoke test for pipe-rush (PipeRush)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof PipeRush !== 'undefined' && PipeRush.update) PipeRush.update(dt);
    if (typeof PipeRush !== 'undefined' && PipeRush.draw) PipeRush.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof PipeRush !== 'undefined' && PipeRush.tap) PipeRush.tap(x,y);
...
// init
try {
  if (typeof PipeRush !== 'undefined' && PipeRush.init) PipeRush.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (pipe-rush): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS pipe-rush');
}
