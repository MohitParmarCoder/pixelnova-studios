...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('shaperecall_best'), 10) || 0; } catch(e) {}
  ShapeRecall.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ShapeRecall.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ShapeRecall.update(dt); }
    ShapeRecall.draw();
    var b = ShapeRecall.getBest ? ShapeRecall.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('shaperecall_best', String(best)); } catch(e) {} }
