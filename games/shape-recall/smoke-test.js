'use strict';
// Headless smoke test for shape-recall (ShapeRecall)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ShapeRecall !== 'undefined' && ShapeRecall.update) ShapeRecall.update(dt);
    if (typeof ShapeRecall !== 'undefined' && ShapeRecall.draw) ShapeRecall.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ShapeRecall !== 'undefined' && ShapeRecall.tap) ShapeRecall.tap(x,y);
...
// init
try {
  if (typeof ShapeRecall !== 'undefined' && ShapeRecall.init) ShapeRecall.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (shape-recall): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS shape-recall');
}
