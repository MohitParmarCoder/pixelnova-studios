...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('orbittap_best'), 10) || 0; } catch(e) {}
  OrbitTap.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); OrbitTap.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { OrbitTap.update(dt); }
    OrbitTap.draw();
    var b = OrbitTap.getBest ? OrbitTap.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('orbittap_best', String(best)); } catch(e) {} }
