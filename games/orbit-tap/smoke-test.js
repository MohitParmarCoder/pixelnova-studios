'use strict';
// Headless smoke test for orbit-tap (OrbitTap)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof OrbitTap !== 'undefined' && OrbitTap.update) OrbitTap.update(dt);
    if (typeof OrbitTap !== 'undefined' && OrbitTap.draw) OrbitTap.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof OrbitTap !== 'undefined' && OrbitTap.tap) OrbitTap.tap(x,y);
...
// init
try {
  if (typeof OrbitTap !== 'undefined' && OrbitTap.init) OrbitTap.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (orbit-tap): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS orbit-tap');
}
