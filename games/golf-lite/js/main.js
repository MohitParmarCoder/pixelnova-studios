...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('golflite_best'), 10) || 0; } catch(e) {}
  GolfLite.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); GolfLite.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { GolfLite.update(dt); }
    GolfLite.draw();
    var b = GolfLite.getBest ? GolfLite.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('golflite_best', String(best)); } catch(e) {} }
