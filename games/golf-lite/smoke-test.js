'use strict';
// Headless smoke test for golf-lite (GolfLite)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof GolfLite !== 'undefined' && GolfLite.update) GolfLite.update(dt);
    if (typeof GolfLite !== 'undefined' && GolfLite.draw) GolfLite.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof GolfLite !== 'undefined' && GolfLite.tap) GolfLite.tap(x,y);
...
// init
try {
  if (typeof GolfLite !== 'undefined' && GolfLite.init) GolfLite.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (golf-lite): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS golf-lite');
}
