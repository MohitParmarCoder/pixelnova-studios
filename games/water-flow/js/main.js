...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('waterflow_best'), 10) || 0; } catch(e) {}
  WaterFlow.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); WaterFlow.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { WaterFlow.update(dt); }
    WaterFlow.draw();
    var b = WaterFlow.getBest ? WaterFlow.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('waterflow_best', String(best)); } catch(e) {} }
