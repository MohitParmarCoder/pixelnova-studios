'use strict';
// Headless smoke test for water-flow (WaterFlow)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof WaterFlow !== 'undefined' && WaterFlow.update) WaterFlow.update(dt);
    if (typeof WaterFlow !== 'undefined' && WaterFlow.draw) WaterFlow.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof WaterFlow !== 'undefined' && WaterFlow.tap) WaterFlow.tap(x,y);
...
// init
try {
  if (typeof WaterFlow !== 'undefined' && WaterFlow.init) WaterFlow.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (water-flow): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
