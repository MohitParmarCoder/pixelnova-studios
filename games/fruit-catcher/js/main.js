...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('fruitcatcher_best'), 10) || 0; } catch(e) {}
  FruitCatcher.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); FruitCatcher.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { FruitCatcher.update(dt); }
    FruitCatcher.draw();
    var b = FruitCatcher.getBest ? FruitCatcher.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('fruitcatcher_best', String(best)); } catch(e) {} }
