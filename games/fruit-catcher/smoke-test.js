'use strict';
// Headless smoke test for fruit-catcher (FruitCatcher)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof FruitCatcher !== 'undefined' && FruitCatcher.update) FruitCatcher.update(dt);
    if (typeof FruitCatcher !== 'undefined' && FruitCatcher.draw) FruitCatcher.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof FruitCatcher !== 'undefined' && FruitCatcher.tap) FruitCatcher.tap(x,y);
...
// init
try {
  if (typeof FruitCatcher !== 'undefined' && FruitCatcher.init) FruitCatcher.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (fruit-catcher): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS fruit-catcher');
}
