...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('magmahop_best'), 10) || 0; } catch(e) {}
  MagmaHop.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); MagmaHop.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { MagmaHop.update(dt); }
    MagmaHop.draw();
    var b = MagmaHop.getBest ? MagmaHop.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('magmahop_best', String(best)); } catch(e) {} }
