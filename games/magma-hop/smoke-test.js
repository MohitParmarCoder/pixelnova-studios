'use strict';
// Headless smoke test for magma-hop (MagmaHop)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof MagmaHop !== 'undefined' && MagmaHop.update) MagmaHop.update(dt);
    if (typeof MagmaHop !== 'undefined' && MagmaHop.draw) MagmaHop.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof MagmaHop !== 'undefined' && MagmaHop.tap) MagmaHop.tap(x,y);
...
// init
try {
  if (typeof MagmaHop !== 'undefined' && MagmaHop.init) MagmaHop.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (magma-hop): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS magma-hop');
}
