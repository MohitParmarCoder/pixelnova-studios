...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('cannonlaunch_best'), 10) || 0; } catch(e) {}
  CannonLaunch.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); CannonLaunch.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { CannonLaunch.update(dt); }
    CannonLaunch.draw();
    var b = CannonLaunch.getBest ? CannonLaunch.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('cannonlaunch_best', String(best)); } catch(e) {} }
