'use strict';
// Headless smoke test for cannon-launch (CannonLaunch)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof CannonLaunch !== 'undefined' && CannonLaunch.update) CannonLaunch.update(dt);
    if (typeof CannonLaunch !== 'undefined' && CannonLaunch.draw) CannonLaunch.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof CannonLaunch !== 'undefined' && CannonLaunch.tap) CannonLaunch.tap(x,y);
...
// init
try {
  if (typeof CannonLaunch !== 'undefined' && CannonLaunch.init) CannonLaunch.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (cannon-launch): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS cannon-launch');
}
