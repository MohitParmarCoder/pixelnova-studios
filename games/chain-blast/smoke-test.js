'use strict';
// Headless smoke test for chain-blast (ChainBlast)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ChainBlast !== 'undefined' && ChainBlast.update) ChainBlast.update(dt);
    if (typeof ChainBlast !== 'undefined' && ChainBlast.draw) ChainBlast.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ChainBlast !== 'undefined' && ChainBlast.tap) ChainBlast.tap(x,y);
...
// init
try {
  if (typeof ChainBlast !== 'undefined' && ChainBlast.init) ChainBlast.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (chain-blast): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS chain-blast');
}
