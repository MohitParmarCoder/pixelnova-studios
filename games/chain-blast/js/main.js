...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('chainblast_best'), 10) || 0; } catch(e) {}
  ChainBlast.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ChainBlast.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ChainBlast.update(dt); }
    ChainBlast.draw();
    var b = ChainBlast.getBest ? ChainBlast.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('chainblast_best', String(best)); } catch(e) {} }
