'use strict';
// Headless smoke test for star-blaster (StarBlaster)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof StarBlaster !== 'undefined' && StarBlaster.update) StarBlaster.update(dt);
    if (typeof StarBlaster !== 'undefined' && StarBlaster.draw) StarBlaster.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof StarBlaster !== 'undefined' && StarBlaster.tap) StarBlaster.tap(x,y);
...
// init
try {
  if (typeof StarBlaster !== 'undefined' && StarBlaster.init) StarBlaster.init(canvasStub, 0);
} catch(e) { errors.push('init: '+e.message.slice(0,80)); }

// idle
frames(60);
// start
tap(195, 780); frames(120);
...
if (uniq.length) {
  console.error('FAIL (star-blaster): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS star-blaster');
}
