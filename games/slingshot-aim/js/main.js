...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('slingshotaim_best'), 10) || 0; } catch(e) {}
  SlingshotAim.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); SlingshotAim.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { SlingshotAim.update(dt); }
    SlingshotAim.draw();
    var b = SlingshotAim.getBest ? SlingshotAim.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('slingshotaim_best', String(best)); } catch(e) {} }
