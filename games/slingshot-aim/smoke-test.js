'use strict';
// Headless smoke test for slingshot-aim (SlingshotAim)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof SlingshotAim !== 'undefined' && SlingshotAim.update) SlingshotAim.update(dt);
    if (typeof SlingshotAim !== 'undefined' && SlingshotAim.draw) SlingshotAim.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof SlingshotAim !== 'undefined' && SlingshotAim.tap) SlingshotAim.tap(x,y);
...
// init
try {
  if (typeof SlingshotAim !== 'undefined' && SlingshotAim.init) SlingshotAim.init(canvasStub, 0);
} catch(e) { errors.push('init: '+e.message.slice(0,80)); }

// idle
frames(60);
// start
tap(80, 680); frames(120);
...
if (uniq.length) {
  console.error('FAIL (slingshot-aim): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS slingshot-aim');
}
