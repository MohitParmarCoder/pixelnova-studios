'use strict';
// Headless smoke test for tap-blast (TapBlast)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof TapBlast !== 'undefined' && TapBlast.update) TapBlast.update(dt);
    if (typeof TapBlast !== 'undefined' && TapBlast.draw) TapBlast.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof TapBlast !== 'undefined' && TapBlast.tap) TapBlast.tap(x,y);
...
// init
try {
  if (typeof TapBlast !== 'undefined' && TapBlast.init) TapBlast.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (tap-blast): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS tap-blast');
}
