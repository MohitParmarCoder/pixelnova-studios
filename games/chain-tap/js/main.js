...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('chaintap_best'), 10) || 0; } catch(e) {}
  ChainTap.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ChainTap.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ChainTap.update(dt); }
    ChainTap.draw();
    var b = ChainTap.getBest ? ChainTap.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('chaintap_best', String(best)); } catch(e) {} }
