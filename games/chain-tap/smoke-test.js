'use strict';
// Headless smoke test for chain-tap (ChainTap)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ChainTap !== 'undefined' && ChainTap.update) ChainTap.update(dt);
    if (typeof ChainTap !== 'undefined' && ChainTap.draw) ChainTap.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ChainTap !== 'undefined' && ChainTap.tap) ChainTap.tap(x,y);
...
// init
try {
  if (typeof ChainTap !== 'undefined' && ChainTap.init) ChainTap.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (chain-tap): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS chain-tap');
}
