'use strict';
// Headless smoke test for gravity-flip (GravityFlip)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof GravityFlip !== 'undefined' && GravityFlip.update) GravityFlip.update(dt);
    if (typeof GravityFlip !== 'undefined' && GravityFlip.draw) GravityFlip.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof GravityFlip !== 'undefined' && GravityFlip.tap) GravityFlip.tap(x,y);
...
// init
try {
  if (typeof GravityFlip !== 'undefined' && GravityFlip.init) GravityFlip.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (gravity-flip): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS gravity-flip');
}
