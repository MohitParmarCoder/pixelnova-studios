...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('bouncemaster_best'), 10) || 0; } catch(e) {}
  BounceMaster.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); BounceMaster.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { BounceMaster.update(dt); }
    BounceMaster.draw();
    var b = BounceMaster.getBest ? BounceMaster.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('bouncemaster_best', String(best)); } catch(e) {} }
