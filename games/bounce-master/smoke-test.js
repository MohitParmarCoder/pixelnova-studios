'use strict';
// Headless smoke test for bounce-master (BounceMaster)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof BounceMaster !== 'undefined' && BounceMaster.update) BounceMaster.update(dt);
    if (typeof BounceMaster !== 'undefined' && BounceMaster.draw) BounceMaster.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof BounceMaster !== 'undefined' && BounceMaster.tap) BounceMaster.tap(x,y);
...
// init
try {
  if (typeof BounceMaster !== 'undefined' && BounceMaster.init) BounceMaster.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (bounce-master): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS bounce-master');
}
