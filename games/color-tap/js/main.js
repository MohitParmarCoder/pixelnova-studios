...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('colortap_best'), 10) || 0; } catch(e) {}
  ColorTap.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); ColorTap.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { ColorTap.update(dt); }
    ColorTap.draw();
    var b = ColorTap.getBest ? ColorTap.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('colortap_best', String(best)); } catch(e) {} }
