'use strict';
// Headless smoke test for color-tap (ColorTap)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ColorTap !== 'undefined' && ColorTap.update) ColorTap.update(dt);
    if (typeof ColorTap !== 'undefined' && ColorTap.draw) ColorTap.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ColorTap !== 'undefined' && ColorTap.tap) ColorTap.tap(x,y);
...
// init
try {
  if (typeof ColorTap !== 'undefined' && ColorTap.init) ColorTap.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (color-tap): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS color-tap');
}
