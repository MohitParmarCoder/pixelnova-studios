'use strict';
// Headless smoke test for reflex-tap (ReflexTap)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof ReflexTap !== 'undefined' && ReflexTap.update) ReflexTap.update(dt);
    if (typeof ReflexTap !== 'undefined' && ReflexTap.draw) ReflexTap.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof ReflexTap !== 'undefined' && ReflexTap.tap) ReflexTap.tap(x,y);
...
// init
try {
  if (typeof ReflexTap !== 'undefined' && ReflexTap.init) ReflexTap.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (reflex-tap): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS reflex-tap');
}
