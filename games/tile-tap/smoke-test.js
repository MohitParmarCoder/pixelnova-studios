'use strict';
// Headless smoke test for tile-tap (TileTap)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof TileTap !== 'undefined' && TileTap.update) TileTap.update(dt);
    if (typeof TileTap !== 'undefined' && TileTap.draw) TileTap.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof TileTap !== 'undefined' && TileTap.tap) TileTap.tap(x,y);
...
// init
try {
  if (typeof TileTap !== 'undefined' && TileTap.init) TileTap.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (tile-tap): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS tile-tap');
}
