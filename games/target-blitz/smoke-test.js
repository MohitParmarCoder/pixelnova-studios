'use strict';
// Headless smoke test for target-blitz (TargetBlitz)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof TargetBlitz !== 'undefined' && TargetBlitz.update) TargetBlitz.update(dt);
    if (typeof TargetBlitz !== 'undefined' && TargetBlitz.draw) TargetBlitz.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof TargetBlitz !== 'undefined' && TargetBlitz.tap) TargetBlitz.tap(x,y);
...
// init
try {
  if (typeof TargetBlitz !== 'undefined' && TargetBlitz.init) TargetBlitz.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (target-blitz): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS target-blitz');
}
