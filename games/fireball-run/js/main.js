...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('fireballrun_best'), 10) || 0; } catch(e) {}
  FireballRun.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); FireballRun.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { FireballRun.update(dt); }
    FireballRun.draw();
    var b = FireballRun.getBest ? FireballRun.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('fireballrun_best', String(best)); } catch(e) {} }
