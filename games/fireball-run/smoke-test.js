'use strict';
// Headless smoke test for fireball-run (FireballRun)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof FireballRun !== 'undefined' && FireballRun.update) FireballRun.update(dt);
    if (typeof FireballRun !== 'undefined' && FireballRun.draw) FireballRun.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof FireballRun !== 'undefined' && FireballRun.tap) FireballRun.tap(x,y);
...
// init
try {
  if (typeof FireballRun !== 'undefined' && FireballRun.init) FireballRun.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (fireball-run): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS fireball-run');
}
