...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('tetrodrop_best'), 10) || 0; } catch(e) {}
  TetroDrop.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); TetroDrop.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { TetroDrop.update(dt); }
    TetroDrop.draw();
    var b = TetroDrop.getBest ? TetroDrop.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('tetrodrop_best', String(best)); } catch(e) {} }
