'use strict';
// Headless smoke test for tetro-drop (TetroDrop)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof TetroDrop !== 'undefined' && TetroDrop.update) TetroDrop.update(dt);
    if (typeof TetroDrop !== 'undefined' && TetroDrop.draw) TetroDrop.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof TetroDrop !== 'undefined' && TetroDrop.tap) TetroDrop.tap(x,y);
...
// init
try {
  if (typeof TetroDrop !== 'undefined' && TetroDrop.init) TetroDrop.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (tetro-drop): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS tetro-drop');
}
