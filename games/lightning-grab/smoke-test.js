'use strict';
// Headless smoke test for lightning-grab (LightningGrab)
const fs = require('fs');
...
function step(dt) {
  try {
    if (typeof LightningGrab !== 'undefined' && LightningGrab.update) LightningGrab.update(dt);
    if (typeof LightningGrab !== 'undefined' && LightningGrab.draw) LightningGrab.draw();
  } catch(e) { errors.push('step: '+e.message.slice(0,80)); }
}
function tap(x,y) {
  try {
    if (typeof LightningGrab !== 'undefined' && LightningGrab.tap) LightningGrab.tap(x,y);
...
// init
try {
  if (typeof LightningGrab !== 'undefined' && LightningGrab.init) LightningGrab.init(canvasStub, 0);
...
if (uniq.length) {
  console.error('FAIL (lightning-grab): '+uniq.slice(0,5).join(' | '));
  process.exit(1);
} else {
  console.log('PASS lightning-grab');
}
