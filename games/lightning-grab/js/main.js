...
(function () {
  ...
  var best = 0;
  try { best = parseInt(localStorage.getItem('lightninggrab_best'), 10) || 0; } catch(e) {}
  LightningGrab.init(canvas, best);
  ...
  document.addEventListener('keydown', function (e) {
    if (e.code === 'Space' || e.code === 'Enter') { e.preventDefault(); LightningGrab.tap(VW / 2, VH / 2); }
  });

  var prev = 0;
  function loop(ts) {
    var dt = Math.min((ts - prev) / 1000, 0.05); prev = ts;
    if (!_paused) { LightningGrab.update(dt); }
    LightningGrab.draw();
    var b = LightningGrab.getBest ? LightningGrab.getBest() : 0;
    if (b > best) { best = b; try { localStorage.setItem('lightninggrab_best', String(best)); } catch(e) {} }
