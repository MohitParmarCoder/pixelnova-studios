# Balance Beam — Ads & Revenue Guide

## Overview
- **Game:** Balance Beam
- **Namespace:** `BalanceBeam`
- **Genre:** Physics Puzzle
- **Estimated session length:** 90s–3min
- **localStorage best-score key:** `balancebeam_best`

## Ad Integration Architecture

Balance Beam uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'balancebeam_best')` |
...
When `offerDoubleScore(score, 'balancebeam_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['balancebeam_best']`
...
### Session Length
Estimated session: 90s–3min. For games under 2 minutes, interstitials every 3 runs is appropriate. For longer games, consider every 2 runs.
