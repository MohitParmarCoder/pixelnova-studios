# Mirror Tap — Ads & Revenue Guide

## Overview
- **Game:** Mirror Tap
- **Namespace:** `MirrorTap`
- **Genre:** Gesture Matching
- **Estimated session length:** 60s–2min
- **localStorage best-score key:** `mirrortap_best`

## Ad Integration Architecture

Mirror Tap uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'mirrortap_best')` |
...
When `offerDoubleScore(score, 'mirrortap_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['mirrortap_best']`
