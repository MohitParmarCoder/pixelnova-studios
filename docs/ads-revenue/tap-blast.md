...
- **localStorage best-score key:** `tapblast_best`

## Ad Integration Architecture

Tap Blast uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'tapblast_best')` |
...
When `offerDoubleScore(score, 'tapblast_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['tapblast_best']`
