# Slingshot Aim — Ads & Revenue Guide

## Overview
- **Game:** Slingshot Aim
- **Namespace:** `SlingshotAim`
- **Genre:** Physics Shooting
- **Estimated session length:** 2–4min
- **localStorage best-score key:** `slingshotaim_best`

## Ad Integration Architecture

Slingshot Aim uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'slingshotaim_best')` |
...
When `offerDoubleScore(score, 'slingshotaim_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['slingshotaim_best']`
...
### Session Length
Estimated session: 2–4min. For games under 2 minutes, interstitials every 3 runs is appropriate. For longer games, consider every 2 runs.
