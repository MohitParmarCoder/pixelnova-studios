# Billiard Aim — Ads & Revenue Guide

## Overview
- **Game:** Billiard Aim
- **Namespace:** `BilliardAim`
- **Genre:** Sports Skill
- **Estimated session length:** 2–5min
- **localStorage best-score key:** `billiardaim_best`

## Ad Integration Architecture

Billiard Aim uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'billiardaim_best')` |
...
When `offerDoubleScore(score, 'billiardaim_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['billiardaim_best']`
