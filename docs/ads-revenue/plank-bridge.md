# Plank Bridge — Ads & Revenue Guide

## Overview
- **Game:** Plank Bridge
- **Namespace:** `PlankBridge`
- **Genre:** Timing Skill
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `plankbridge_best`

## Ad Integration Architecture

Plank Bridge uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'plankbridge_best')` |
...
When `offerDoubleScore(score, 'plankbridge_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['plankbridge_best']`
