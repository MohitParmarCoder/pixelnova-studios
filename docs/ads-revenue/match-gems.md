...
- **localStorage best-score key:** `matchgems_best`

## Ad Integration Architecture

Match Gems uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'matchgems_best')` |
...
When `offerDoubleScore(score, 'matchgems_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['matchgems_best']`
