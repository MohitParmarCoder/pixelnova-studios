# Pinball Lite — Ads & Revenue Guide

## Overview
- **Game:** Pinball Lite
- **Namespace:** `PinballLite`
- **Genre:** Physics Pinball
- **Estimated session length:** 2–5min
- **localStorage best-score key:** `pinballlite_best`

## Ad Integration Architecture

Pinball Lite uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'pinballlite_best')` |
...
When `offerDoubleScore(score, 'pinballlite_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['pinballlite_best']`
