# Shadow Slide — Ads & Revenue Guide

## Overview
- **Game:** Shadow Slide
- **Namespace:** `ShadowSlide`
- **Genre:** Neon Platformer
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `shadowslide_best`

## Ad Integration Architecture

Shadow Slide uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'shadowslide_best')` |
...
When `offerDoubleScore(score, 'shadowslide_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['shadowslide_best']`
