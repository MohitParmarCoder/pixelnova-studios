# Spike Field — Ads & Revenue Guide

## Overview
- **Game:** Spike Field
- **Namespace:** `SpikeField`
- **Genre:** Physics Climb
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `spikefield_best`

## Ad Integration Architecture

Spike Field uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'spikefield_best')` |
...
When `offerDoubleScore(score, 'spikefield_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['spikefield_best']`
