# Pipe Rush — Ads & Revenue Guide

## Overview
- **Game:** Pipe Rush
- **Namespace:** `PipeRush`
- **Genre:** Flappy Variant
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `piperush_best`

## Ad Integration Architecture

Pipe Rush uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'piperush_best')` |
...
When `offerDoubleScore(score, 'piperush_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['piperush_best']`
