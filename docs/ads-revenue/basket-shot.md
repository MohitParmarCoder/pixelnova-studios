# Basket Shot — Ads & Revenue Guide

## Overview
- **Game:** Basket Shot
- **Namespace:** `BasketShot`
- **Genre:** Sports Arcade
- **Estimated session length:** 60s–2min
- **localStorage best-score key:** `basketshot_best`

## Ad Integration Architecture

Basket Shot uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'basketshot_best')` |
...
When `offerDoubleScore(score, 'basketshot_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['basketshot_best']`
