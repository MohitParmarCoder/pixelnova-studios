# Sequence Game — Ads & Revenue Guide

## Overview
- **Game:** Sequence Game
- **Namespace:** `SequenceGame`
- **Genre:** Mastermind Puzzle
- **Estimated session length:** 3–10min
- **localStorage best-score key:** `sequencegame_best`

## Ad Integration Architecture

Sequence Game uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'sequencegame_best')` |
...
When `offerDoubleScore(score, 'sequencegame_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['sequencegame_best']`
...
### Session Length
Estimated session: 3–10min. For games under 2 minutes, interstitials every 3 runs is appropriate. For longer games, consider every 2 runs.
