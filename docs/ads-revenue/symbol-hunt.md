# Symbol Hunt — Ads & Revenue Guide

## Overview
- **Game:** Symbol Hunt
- **Namespace:** `SymbolHunt`
- **Genre:** Visual Search
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `symbolhunt_best`

## Ad Integration Architecture

Symbol Hunt uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'symbolhunt_best')` |
...
When `offerDoubleScore(score, 'symbolhunt_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['symbolhunt_best']`
