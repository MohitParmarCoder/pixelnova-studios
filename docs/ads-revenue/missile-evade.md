# Missile Evade — Ads & Revenue Guide

## Overview
- **Game:** Missile Evade
- **Namespace:** `MissileEvade`
- **Genre:** Defense Shooter
- **Estimated session length:** 2–5min
- **localStorage best-score key:** `missileevade_best`

## Ad Integration Architecture

Missile Evade uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'missileevade_best')` |
...
When `offerDoubleScore(score, 'missileevade_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['missileevade_best']`
