# Fruit Catcher — Ads & Revenue Guide
...
- **localStorage best-score key:** `fruitcatcher_best`

## Ad Integration Architecture

Fruit Catcher uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'fruitcatcher_best')` |
...
When `offerDoubleScore(score, 'fruitcatcher_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['fruitcatcher_best']`
