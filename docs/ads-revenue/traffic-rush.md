# Traffic Rush — Ads & Revenue Guide
...
- **localStorage best-score key:** `trafficrush_best`

## Ad Integration Architecture

Traffic Rush uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'trafficrush_best')` |
...
When `offerDoubleScore(score, 'trafficrush_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['trafficrush_best']`
