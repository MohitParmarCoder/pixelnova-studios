# Snowball Catch — Ads & Revenue Guide

## Overview
- **Game:** Snowball Catch
- **Namespace:** `SnowballCatch`
- **Genre:** Fishing Game
- **Estimated session length:** 2–5min
- **localStorage best-score key:** `snowballcatch_best`

## Ad Integration Architecture

Snowball Catch uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'snowballcatch_best')` |
...
When `offerDoubleScore(score, 'snowballcatch_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['snowballcatch_best']`
...
### Session Length
Estimated session: 2–5min. For games under 2 minutes, interstitials every 3 runs is appropriate. For longer games, consider every 2 runs.
