# Pulse Dodge — Ads & Revenue Guide

## Overview
- **Game:** Pulse Dodge
- **Namespace:** `PulseDodge`
- **Genre:** Orbital Dodge
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `pulsedodge_best`

## Ad Integration Architecture

Pulse Dodge uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'pulsedodge_best')` |
...
When `offerDoubleScore(score, 'pulsedodge_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['pulsedodge_best']`
