# Bowling Strike — Ads & Revenue Guide

## Overview
- **Game:** Bowling Strike
- **Namespace:** `BowlingStrike`
- **Genre:** Sports Physics
- **Estimated session length:** 2–4min
- **localStorage best-score key:** `bowlingstrike_best`

## Ad Integration Architecture

Bowling Strike uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'bowlingstrike_best')` |
...
When `offerDoubleScore(score, 'bowlingstrike_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['bowlingstrike_best']`
