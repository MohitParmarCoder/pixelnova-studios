# Color Tap — Ads & Revenue Guide

## Overview
- **Game:** Color Tap
- **Namespace:** `ColorTap`
- **Genre:** Cognitive Reflex
- **Estimated session length:** 60s–2min
- **localStorage best-score key:** `colortap_best`

## Ad Integration Architecture

Color Tap uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'colortap_best')` |
...
When `offerDoubleScore(score, 'colortap_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['colortap_best']`
