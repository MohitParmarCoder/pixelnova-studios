# Cave Runner — Ads & Revenue Guide

## Overview
- **Game:** Cave Runner
- **Namespace:** `CaveRunner`
- **Genre:** Grid Exploration
- **Estimated session length:** 3–8min
- **localStorage best-score key:** `caverunner_best`

## Ad Integration Architecture

Cave Runner uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'caverunner_best')` |
...
When `offerDoubleScore(score, 'caverunner_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['caverunner_best']`
