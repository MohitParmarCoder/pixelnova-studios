# Lightning Grab — Ads & Revenue Guide

## Overview
- **Game:** Lightning Grab
- **Namespace:** `LightningGrab`
- **Genre:** Color Reflex
- **Estimated session length:** 60s–2min
- **localStorage best-score key:** `lightninggrab_best`

## Ad Integration Architecture

Lightning Grab uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'lightninggrab_best')` |
...
When `offerDoubleScore(score, 'lightninggrab_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['lightninggrab_best']`
