# Electric Dash — Ads & Revenue Guide

## Overview
- **Game:** Electric Dash
- **Namespace:** `ElectricDash`
- **Genre:** Circuit Puzzle
- **Estimated session length:** 3–8min
- **localStorage best-score key:** `electricdash_best`

## Ad Integration Architecture

Electric Dash uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'electricdash_best')` |
...
When `offerDoubleScore(score, 'electricdash_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['electricdash_best']`
