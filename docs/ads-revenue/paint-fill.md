# Paint Fill — Ads & Revenue Guide

## Overview
- **Game:** Paint Fill
- **Namespace:** `PaintFill`
- **Genre:** Flood Fill Puzzle
- **Estimated session length:** 3–8min
- **localStorage best-score key:** `paintfill_best`

## Ad Integration Architecture

Paint Fill uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'paintfill_best')` |
...
When `offerDoubleScore(score, 'paintfill_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['paintfill_best']`
