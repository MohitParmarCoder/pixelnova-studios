# Chain Blast — Ads & Revenue Guide

## Overview
- **Game:** Chain Blast
- **Namespace:** `ChainBlast`
- **Genre:** Chain Reaction
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `chainblast_best`

## Ad Integration Architecture

Chain Blast uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'chainblast_best')` |
...
When `offerDoubleScore(score, 'chainblast_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['chainblast_best']`
