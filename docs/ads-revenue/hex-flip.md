...
- **localStorage best-score key:** `hexflip_best`

## Ad Integration Architecture

Hex Flip uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'hexflip_best')` |
...
When `offerDoubleScore(score, 'hexflip_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['hexflip_best']`
