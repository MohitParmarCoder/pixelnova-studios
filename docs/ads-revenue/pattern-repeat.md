# Pattern Repeat — Ads & Revenue Guide

## Overview
- **Game:** Pattern Repeat
- **Namespace:** `PatternRepeat`
- **Genre:** Memory Simon
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `patternrepeat_best`

## Ad Integration Architecture

Pattern Repeat uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'patternrepeat_best')` |
...
When `offerDoubleScore(score, 'patternrepeat_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['patternrepeat_best']`
