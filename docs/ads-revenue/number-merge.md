# Number Merge — Ads & Revenue Guide

## Overview
- **Game:** Number Merge
- **Namespace:** `NumberMerge`
- **Genre:** Puzzle Snake
- **Estimated session length:** 2–5min
- **localStorage best-score key:** `numbermerge_best`

## Ad Integration Architecture

Number Merge uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'numbermerge_best')` |
...
When `offerDoubleScore(score, 'numbermerge_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['numbermerge_best']`
