# Shape Recall — Ads & Revenue Guide

## Overview
- **Game:** Shape Recall
- **Namespace:** `ShapeRecall`
- **Genre:** Tangram Puzzle
- **Estimated session length:** 3–8min
- **localStorage best-score key:** `shaperecall_best`

## Ad Integration Architecture

Shape Recall uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'shaperecall_best')` |
...
When `offerDoubleScore(score, 'shaperecall_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['shaperecall_best']`
...
### Session Length
Estimated session: 3–8min. For games under 2 minutes, interstitials every 3 runs is appropriate. For longer games, consider every 2 runs.
