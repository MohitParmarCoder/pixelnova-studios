# Orb Collector — Ads & Revenue Guide

## Overview
- **Game:** Orb Collector
- **Namespace:** `OrbCollector`
- **Genre:** Collect Arcade
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `orbcollector_best`

## Ad Integration Architecture

Orb Collector uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'orbcollector_best')` |
...
When `offerDoubleScore(score, 'orbcollector_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['orbcollector_best']`
