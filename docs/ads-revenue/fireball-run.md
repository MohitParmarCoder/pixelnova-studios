# Fireball Run — Ads & Revenue Guide

## Overview
- **Game:** Fireball Run
- **Namespace:** `FireballRun`
- **Genre:** Tower Defense
- **Estimated session length:** 2–5min
- **localStorage best-score key:** `fireballrun_best`

## Ad Integration Architecture

Fireball Run uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'fireballrun_best')` |
...
When `offerDoubleScore(score, 'fireballrun_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['fireballrun_best']`
