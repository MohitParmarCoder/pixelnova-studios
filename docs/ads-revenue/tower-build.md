# Tower Build — Ads & Revenue Guide

## Overview
- **Game:** Tower Build
- **Namespace:** `TowerBuild`
- **Genre:** Stacking Arcade
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `towerbuild_best`

## Ad Integration Architecture

Tower Build uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'towerbuild_best')` |
...
When `offerDoubleScore(score, 'towerbuild_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['towerbuild_best']`
...
### Session Length
Estimated session: 60s–3min. For games under 2 minutes, interstitials every 3 runs is appropriate. For longer games, consider every 2 runs.
