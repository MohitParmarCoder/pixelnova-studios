# Spiral Draw — Ads & Revenue Guide

## Overview
- **Game:** Spiral Draw
- **Namespace:** `SpiralDraw`
- **Genre:** Order Tap
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `spiraldraw_best`

## Ad Integration Architecture

Spiral Draw uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'spiraldraw_best')` |
...
When `offerDoubleScore(score, 'spiraldraw_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['spiraldraw_best']`
