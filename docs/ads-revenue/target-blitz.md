# Target Blitz — Ads & Revenue Guide

## Overview
- **Game:** Target Blitz
- **Namespace:** `TargetBlitz`
- **Genre:** Timed Score
- **Estimated session length:** 30s–2min
- **localStorage best-score key:** `targetblitz_best`

## Ad Integration Architecture

Target Blitz uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'targetblitz_best')` |
...
When `offerDoubleScore(score, 'targetblitz_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['targetblitz_best']`
...
### Session Length
Estimated session: 30s–2min. For games under 2 minutes, interstitials every 3 runs is appropriate. For longer games, consider every 2 runs.
