# Rooftop Run — Ads & Revenue Guide

## Overview
- **Game:** Rooftop Run
- **Namespace:** `RooftopRun`
- **Genre:** Platformer
- **Estimated session length:** 60s–3min
- **localStorage best-score key:** `rooftoprun_best`

## Ad Integration Architecture

Rooftop Run uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'rooftoprun_best')` |
...
When `offerDoubleScore(score, 'rooftoprun_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['rooftoprun_best']`
