# Potion Grab — Ads & Revenue Guide

## Overview
- **Game:** Potion Grab
- **Namespace:** `PotionGrab`
- **Genre:** Color Matching
- **Estimated session length:** 2–5min
- **localStorage best-score key:** `potiongrab_best`

## Ad Integration Architecture

Potion Grab uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'potiongrab_best')` |
...
When `offerDoubleScore(score, 'potiongrab_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['potiongrab_best']`
