# Cannon Launch — Ads & Revenue Guide

## Overview
- **Game:** Cannon Launch
- **Namespace:** `CannonLaunch`
- **Genre:** Physics Shooting
- **Estimated session length:** 2–4min
- **localStorage best-score key:** `cannonlaunch_best`

## Ad Integration Architecture

Cannon Launch uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'cannonlaunch_best')` |
...
When `offerDoubleScore(score, 'cannonlaunch_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['cannonlaunch_best']`
