# Crystal Stack — Ads & Revenue Guide

## Overview
- **Game:** Crystal Stack
- **Namespace:** `CrystalStack`
- **Genre:** Sort Puzzle
- **Estimated session length:** 3–8min
- **localStorage best-score key:** `crystalstack_best`

## Ad Integration Architecture

Crystal Stack uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'crystalstack_best')` |
...
When `offerDoubleScore(score, 'crystalstack_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['crystalstack_best']`
