# Peg Drop — Ads & Revenue Guide

## Overview
- **Game:** Peg Drop
- **Namespace:** `PegDrop`
- **Genre:** Physics Plinko
- **Estimated session length:** 2–4min
- **localStorage best-score key:** `pegdrop_best`

## Ad Integration Architecture

Peg Drop uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'pegdrop_best')` |
...
When `offerDoubleScore(score, 'pegdrop_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['pegdrop_best']`
