# Color Burst — Ads & Revenue Guide

## Overview
- **Game:** Color Burst
- **Namespace:** `ColorBurst`
- **Genre:** Color Match Arcade
- **Estimated session length:** 60s–2min
- **localStorage best-score key:** `colorburst_best`

## Ad Integration Architecture

Color Burst uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'colorburst_best')` |
...
When `offerDoubleScore(score, 'colorburst_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['colorburst_best']`
