# Gravity Maze — Ads & Revenue Guide

## Overview
- **Game:** Gravity Maze
- **Namespace:** `GravityMaze`
- **Genre:** Physics Puzzle
- **Estimated session length:** 2–5min
- **localStorage best-score key:** `gravitymaze_best`

## Ad Integration Architecture

Gravity Maze uses the shared `AdManager` module (`js/ads.js`). The adapter is selected via `config.adapter` at the top of `ads.js`.
...
| DEAD screen offer | `AdManager.offerDoubleScore(score, 'gravitymaze_best')` |
...
When `offerDoubleScore(score, 'gravitymaze_best')` is called:
1. A DOM overlay renders: "Watch Ad — Double Your Score?"
2. Player taps "Watch Ad" → rewarded ad plays
3. On ad completion: `score × 2` is saved to `localStorage['gravitymaze_best']`
