# Potion Grab — Codebase Reference

## Overview
Potion Grab is a color-mix slider game. 3 RGB sliders; drag thumbs to mix a color to match the target color. Score based on accuracy. Multiple rounds with shrinking time per round. The global namespace is `PotionGrab` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `PotionGrab`
...
| `best` | All-time best score (from localStorage `potiongrab_best`) |
...
| Key | Content |
|---|---|
| `potiongrab_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'potiongrab_best')` |
