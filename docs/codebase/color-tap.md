# Color Tap — Codebase Reference

## Overview
Color Tap is a Stroop color recognition game. A color word is displayed in a possibly different ink color. Player must tap the button matching the INK COLOR (not the word). Stroop interference increases difficulty. The global namespace is `ColorTap` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `ColorTap`
...
| `best` | All-time best score (from localStorage `colortap_best`) |
...
| Key | Content |
|---|---|
| `colortap_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'colortap_best')` |
