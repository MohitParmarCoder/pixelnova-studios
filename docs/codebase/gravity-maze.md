# Gravity Maze — Codebase Reference

## Overview
Gravity Maze is a flip-gravity ball maze game. Ball in a maze. Tap to flip gravity direction (up/down). Navigate ball from start to exit star. Wall collisions end the run. Each level has unique wall/passage layout. The global namespace is `GravityMaze` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `GravityMaze`
...
| `best` | All-time best score (from localStorage `gravitymaze_best`) |
...
| Key | Content |
|---|---|
| `gravitymaze_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'gravitymaze_best')` |
