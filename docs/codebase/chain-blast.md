# Chain Blast — Codebase Reference

## Overview
Chain Blast is a chain-reaction bubble matching game. Colored bubbles float on screen. Tap a bubble to highlight it; tap adjacent same-color bubbles to chain. Larger chains give bonus points. Bubbles cap at MAX_BUBBLES. The global namespace is `ChainBlast` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `ChainBlast`
...
| `best` | All-time best score (from localStorage `chainblast_best`) |
...
| Key | Content |
|---|---|
| `chainblast_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'chainblast_best')` |
