# Cave Runner — Codebase Reference

## Overview
Cave Runner is a grid-based dungeon mining game. Player navigates a grid cave selecting cells — rock, gem, gold, bomb, or empty. Gems and gold give points; bombs remove a life. The grid scrolls as the player descends. The global namespace is `CaveRunner` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `CaveRunner`
...
| `best` | All-time best score (from localStorage `caverunner_best`) |
...
| Key | Content |
|---|---|
| `caverunner_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'caverunner_best')` |
