# City Stack — Codebase Reference

## Overview
City Stack is a moving-block city stacking game. A block slides horizontally; tap to drop it onto the stack. Overhanging portion is trimmed. Game over when width reduces to zero or lives run out. Levels scroll upward. The global namespace is `CityStack` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `CityStack`
...
| `best` | All-time best score (from localStorage `citystack_best`) |
...
| Key | Content |
|---|---|
| `citystack_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'citystack_best')` |
