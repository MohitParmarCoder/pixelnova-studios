# Orb Collector — Codebase Reference

## Overview
Orb Collector is a drag-to-collect orbs game. Player controls a collector orb by touching/dragging. Colored orbs float across the screen; collect good ones, avoid red bad ones. Score = orbs collected. The global namespace is `OrbCollector` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `OrbCollector`
...
| `best` | All-time best score (from localStorage `orbcollector_best`) |
...
| Key | Content |
|---|---|
| `orbcollector_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'orbcollector_best')` |
