# Crystal Stack — Codebase Reference

## Overview
Crystal Stack is a tube ball color-sort puzzle game. 5 test tubes each holding colored balls. Tap a tube to pick up the top ball; tap another to place it. Goal: sort each tube to hold only one color. Wrong moves cost a life. The global namespace is `CrystalStack` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `CrystalStack`
...
| `best` | All-time best score (from localStorage `crystalstack_best`) |
...
| Key | Content |
|---|---|
| `crystalstack_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'crystalstack_best')` |
