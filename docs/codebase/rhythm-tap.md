# Rhythm Tap — Codebase Reference

## Overview
Rhythm Tap is a falling circles rhythm tap game. Circles fall in 3 lanes. Tap when a circle reaches the bottom beat line. Perfect/Good timing windows. Circles speed up over time. Lives lost on miss or very early hit. The global namespace is `RhythmTap` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `RhythmTap`
...
| `best` | All-time best score (from localStorage `rhythmtap_best`) |
...
| Key | Content |
|---|---|
| `rhythmtap_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'rhythmtap_best')` |
