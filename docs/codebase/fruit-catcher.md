# Fruit Catcher — Codebase Reference

## Overview
Fruit Catcher is a slash-fruits bomb-avoid game. Fruits fall from the top; tap to slash/collect. Bombs mixed in — tapping a bomb costs a life. Multi-fruit combos score bonus points. Lives capped at 1 loss per frame. The global namespace is `FruitCatcher` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `FruitCatcher`
...
| `best` | All-time best score (from localStorage `fruitcatcher_best`) |
...
| Key | Content |
|---|---|
| `fruitcatcher_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'fruitcatcher_best')` |
