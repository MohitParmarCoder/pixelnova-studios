# Whack Mole — Codebase Reference

## Overview
Whack Mole is a whack-a-mole 3×3 game. 9 holes in 3×3 grid. Moles pop up briefly; tap the mole body to whack it. Miss (mole retreats untouched) = life lost. Moles appear faster and for shorter windows with score. The global namespace is `WhackMole` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `WhackMole`
...
| `best` | All-time best score (from localStorage `whackmole_best`) |
...
| Key | Content |
|---|---|
| `whackmole_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'whackmole_best')` |
