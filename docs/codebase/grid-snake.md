# Grid Snake — Codebase Reference

## Overview
Grid Snake is a numeric-value snake eat game. Classic snake on a 9×14 grid. Food tiles have numeric values. Snake head must eat tiles in ascending numeric order. Eating wrong value or self = game over. Head value doubles on each eat. The global namespace is `GridSnake` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `GridSnake`
...
| `best` | All-time best score (from localStorage `gridsnake_best`) |
...
| Key | Content |
|---|---|
| `gridsnake_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'gridsnake_best')` |
