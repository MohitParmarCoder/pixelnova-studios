# Orbit Tap — Codebase Reference

## Overview
Orbit Tap is a orbital planet destruction game. Planets orbit a central sun. Tap a planet to destroy it before it completes its orbit. Missing a planet (it completes orbit) costs a life. Orbit speeds increase with score. The global namespace is `OrbitTap` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `OrbitTap`
...
| `best` | All-time best score (from localStorage `orbittap_best`) |
...
| Key | Content |
|---|---|
| `orbittap_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'orbittap_best')` |
