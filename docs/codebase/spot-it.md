# Spot It — Codebase Reference

## Overview
Spot It is a find the odd symbol game. 4×4 grid of symbols; all match a pattern except one odd one. Tap the odd symbol before timer runs out (3 seconds). Faster taps give higher score. Lives lost on wrong tap or timeout. The global namespace is `SpotIt` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `SpotIt`
...
| `best` | All-time best score (from localStorage `spotit_best`) |
...
| Key | Content |
|---|---|
| `spotit_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'spotit_best')` |
