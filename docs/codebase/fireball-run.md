# Fireball Run — Codebase Reference

## Overview
Fireball Run is a castle defense arrow shooter game. Castle on left; enemies march from the right. Tap anywhere to fire an arrow in arc trajectory from the castle tower. Enemies reaching the left edge reduce castle HP (lives). The global namespace is `FireballRun` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `FireballRun`
...
| `best` | All-time best score (from localStorage `fireballrun_best`) |
...
| Key | Content |
|---|---|
| `fireballrun_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'fireballrun_best')` |
