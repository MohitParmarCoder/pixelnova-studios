# Stack Tower — Codebase Reference

## Overview
Stack Tower is a precision neon block stacker game. Neon block slides left/right. Tap to drop it onto the stack. Overhang is trimmed each layer. Score = layers stacked. Perfect alignment gives bonus. Stack ends when block too narrow. The global namespace is `StackTower` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `StackTower`
...
| `best` | All-time best score (from localStorage `stacktower_best`) |
...
| Key | Content |
|---|---|
| `stacktower_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'stacktower_best')` |
