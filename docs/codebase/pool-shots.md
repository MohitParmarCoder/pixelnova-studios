# Pool Shots — Codebase Reference

## Overview
Pool Shots is a disc sliding zone score game. Disc launched from bottom; aim to land in scoring zones. Two bumper pegs deflect trajectory. Zones score 1/2/3 pts. Score = total pts across all rounds. The global namespace is `PoolShots` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `PoolShots`
...
| `best` | All-time best score (from localStorage `poolshots_best`) |
...
| Key | Content |
|---|---|
| `poolshots_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'poolshots_best')` |
