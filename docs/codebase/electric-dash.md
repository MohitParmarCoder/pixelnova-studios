# Electric Dash — Codebase Reference

## Overview
Electric Dash is a wire rotation circuit puzzle game. A 5×7 grid of wire segments (straight, corner, T, cross). Tap a cell to rotate it 90°. Goal: connect power source to all endpoints forming a complete circuit. Timer counts down. The global namespace is `ElectricDash` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `ElectricDash`
...
| `best` | All-time best score (from localStorage `electricdash_best`) |
...
| Key | Content |
|---|---|
| `electricdash_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'electricdash_best')` |
