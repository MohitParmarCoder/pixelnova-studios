# Symbol Hunt — Codebase Reference

## Overview
Symbol Hunt is a find target symbol in grid game. A target symbol is shown at top. A 5×7 grid of symbols appears. Player must tap all cells matching the target before timer runs out. Wrong tap = life lost. The global namespace is `SymbolHunt` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `SymbolHunt`
...
| `best` | All-time best score (from localStorage `symbolhunt_best`) |
...
| Key | Content |
|---|---|
| `symbolhunt_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'symbolhunt_best')` |
