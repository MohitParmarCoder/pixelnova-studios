# Hex Flip — Codebase Reference

## Overview
Hex Flip is a hexagon color-match tap game. 7×9 hexagonal grid with 3 colors. Tap a hex to cycle its color. Score points for completing rings of matching colors. 60-second timer. Combo chains score multiplier. The global namespace is `HexFlip` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `HexFlip`
...
| `best` | All-time best score (from localStorage `hexflip_best`) |
...
| Key | Content |
|---|---|
| `hexflip_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'hexflip_best')` |
