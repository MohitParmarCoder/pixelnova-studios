# Shape Recall — Codebase Reference

## Overview
Shape Recall is a tangram piece placement game. 5 tangram-style pieces must be placed into their matching slots within a silhouette shape. Tap piece to select; tap target slot to place. Wrong slot = life lost. 5 puzzles. The global namespace is `ShapeRecall` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `ShapeRecall`
...
| `best` | All-time best score (from localStorage `shaperecall_best`) |
...
| Key | Content |
|---|---|
| `shaperecall_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'shaperecall_best')` |
