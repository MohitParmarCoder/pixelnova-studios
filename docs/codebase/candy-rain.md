# Candy Rain — Codebase Reference

## Overview
Candy Rain is a number-pair matching game. Numbered bubbles float on screen; player must tap pairs that sum to the randomly chosen target number. Timer counts down each round. The global namespace is `CandyRain` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `CandyRain`
...
| `best` | All-time best score (from localStorage `candyrain_best`) |
...
| Key | Content |
|---|---|
| `candyrain_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'candyrain_best')` |
