# Peg Drop — Codebase Reference

## Overview
Peg Drop is a Plinko peg-board game. Drag to position drop point. Release ball to fall through peg field. Ball lands in numbered slot (1–10). Score = sum of all ball landings across 10 balls. Pegs provide randomization. The global namespace is `PegDrop` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `PegDrop`
...
| `ballsLeft` | Player resource (ballsLeft) |
| `best` | All-time best score (from localStorage `pegdrop_best`) |
...
| Key | Content |
|---|---|
| `pegdrop_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'pegdrop_best')` |
