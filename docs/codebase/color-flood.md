# Color Flood — Codebase Reference

## Overview
Color Flood is a 15-puzzle slide tile game. A 4×4 grid with tiles 1–15 plus one blank space. Slide tiles into numerical order using the blank space. Score based on rounds solved; lives lost when move limit (200) is exceeded. The global namespace is `ColorFlood` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `ColorFlood`
...
| `best` | All-time best score (from localStorage `colorflood_best`) |
...
| Key | Content |
|---|---|
| `colorflood_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'colorflood_best')` |
