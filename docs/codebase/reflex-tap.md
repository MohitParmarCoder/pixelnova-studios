# Reflex Tap — Codebase Reference

## Overview
Reflex Tap is a spinning ring hit-zone game. A ring spins around a center. A colored hit zone (arc) on the ring must be tapped when a ball (or marker) enters it. Speed increases; hit zone arc shrinks. Lives lost on miss. The global namespace is `ReflexTap` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `ReflexTap`
...
| `best` | All-time best score (from localStorage `reflextap_best`) |
...
| Key | Content |
|---|---|
| `reflextap_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'reflextap_best')` |
