# Water Flow — Codebase Reference

## Overview
Water Flow is a pipe-rotation flow puzzle game. Grid of pipe segments. Tap to rotate each pipe. Goal: create a connected path from source to sink. Water flows and visual feedback shows connected pipes. Timed rounds. The global namespace is `WaterFlow` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `WaterFlow`
...
| `best` | All-time best score (from localStorage `waterflow_best`) |
...
| Key | Content |
|---|---|
| `waterflow_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'waterflow_best')` |
