# Snowball Catch — Codebase Reference

## Overview
Snowball Catch is a fishing hook catch game. Fishing hook drops into water; tap to release/retract. Fish swim at various depths; hook must reach fish depth. Catch fish = score; miss 3 consecutive = life lost. The global namespace is `SnowballCatch` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `SnowballCatch`
...
| `best` | All-time best score (from localStorage `snowballcatch_best`) |
...
| Key | Content |
|---|---|
| `snowballcatch_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'snowballcatch_best')` |
