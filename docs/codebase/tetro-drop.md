# Tetro Drop — Codebase Reference

## Overview
Tetro Drop is a rotating board ball-into-hole game. Circular board with 4 holes (green = +10 pts, red = -1 life). A rotating arm controls a ball's orbit. Tap to freeze arm momentarily, guiding the ball into green holes. The global namespace is `TetroDrop` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `TetroDrop`
...
| `best` | All-time best score (from localStorage `tetrodrop_best`) |
...
| Key | Content |
|---|---|
| `tetrodrop_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'tetrodrop_best')` |
