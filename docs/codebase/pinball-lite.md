# Pinball Lite — Codebase Reference

## Overview
Pinball Lite is a physics pinball game. Tap left/right to control two flippers. Ball bounces off bumpers, targets, and walls scoring points. Ball falls below flippers = life lost. Bumpers flash and score on impact. The global namespace is `PinballLite` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `PinballLite`
...
| `best` | All-time best score (from localStorage `pinballlite_best`) |
...
| Key | Content |
|---|---|
| `pinballlite_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'pinballlite_best')` |
