# Dot Link — Codebase Reference

## Overview
Dot Link is a flow-free connect-dots puzzle game. Grid puzzle: connect same-color dot pairs with a path that fills the entire grid. Paths cannot cross. Timed puzzles; score = puzzles solved. Grid size increases with difficulty. The global namespace is `DotLink` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `DotLink`
...
| `best` | All-time best score (from localStorage `dotlink_best`) |
...
| Key | Content |
|---|---|
| `dotlink_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'dotlink_best')` |
