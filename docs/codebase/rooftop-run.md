# Rooftop Run — Codebase Reference

## Overview
Rooftop Run is a timing-jump platformer game. Platforms scroll upward. Player must tap at the right moment to jump from platform to platform. Perfect timing gives bonus. Miss = life lost. Platform spacing varies. The global namespace is `RooftopRun` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `RooftopRun`
...
| `best` | All-time best score (from localStorage `rooftoprun_best`) |
...
| Key | Content |
|---|---|
| `rooftoprun_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'rooftoprun_best')` |
