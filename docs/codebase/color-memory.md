# Color Memory — Codebase Reference

## Overview
Color Memory is a piano melody memory game. A melody plays on 8 colored piano keys. Player must replay it in exact order. Sequence grows by 1 note each round. Lives lost for wrong key presses. The global namespace is `ColorMemory` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `ColorMemory`
...
| `best` | All-time best score (from localStorage `colormemory_best`) |
...
| Key | Content |
|---|---|
| `colormemory_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'colormemory_best')` |
