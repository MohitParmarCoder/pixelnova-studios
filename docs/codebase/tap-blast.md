# Tap Blast — Codebase Reference

## Overview
Tap Blast is a timed circle tap game. Circles appear at random positions and shrink over time. Tap before they disappear. Each miss (expired circle) = life lost. Spawn rate increases and max-life shrinks with score. The global namespace is `TapBlast` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `TapBlast`
...
| `best` | All-time best score (from localStorage `tapblast_best`) |
...
| Key | Content |
|---|---|
| `tapblast_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'tapblast_best')` |
