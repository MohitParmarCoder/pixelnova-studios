# Line Breaker — Codebase Reference

## Overview
Line Breaker is a ball-and-paddle brick breaker game. Classic breakout: ball bounces off paddle and bricks. Tap/drag to move paddle. Bricks have HP (1–3). New brick rows added as score increases. Ball speed increases over time. The global namespace is `LineBreaker` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `LineBreaker`
...
| `best` | All-time best score (from localStorage `linebreaker_best`) |
...
| Key | Content |
|---|---|
| `linebreaker_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'linebreaker_best')` |
