# Golf Lite — Codebase Reference

## Overview
Golf Lite is a mini-golf tap-aim-shoot game. Two-tap aim system: first tap sets direction, second tap sets power. Ball rolls under friction toward hole. Score = total putts across 10 holes. Par system tracks efficiency. The global namespace is `GolfLite` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `GolfLite`
...
| `best` | All-time best score (from localStorage `golflite_best`) |
...
| Key | Content |
|---|---|
| `golflite_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'golflite_best')` |
