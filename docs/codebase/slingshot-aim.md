# Slingshot Aim — Codebase Reference

## Overview
Slingshot Aim is a slingshot physics game. Tap to set aim direction from fixed slingshot. Ball flies in arc. 5 targets on screen; score per target hit. 10 shots per game. Tapping sets angle; physics handles trajectory. The global namespace is `SlingshotAim` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `SlingshotAim`
...
| `shotsLeft` | Player resource (shotsLeft) |
| `best` | All-time best score (from localStorage `slingshotaim_best`) |
...
| Key | Content |
|---|---|
| `slingshotaim_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'slingshotaim_best')` |
