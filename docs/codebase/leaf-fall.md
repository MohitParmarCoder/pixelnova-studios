# Leaf Fall — Codebase Reference

## Overview
Leaf Fall is a tap moving bugs game. Bugs move horizontally across the screen. Tap to squash them before they exit. Gold bugs score 5 pts; regular bugs score 1 pt. Bug count and speed increase with score. The global namespace is `LeafFall` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `LeafFall`
...
| `best` | All-time best score (from localStorage `leaffall_best`) |
...
| Key | Content |
|---|---|
| `leaffall_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'leaffall_best')` |
