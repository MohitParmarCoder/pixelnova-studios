# Gem Collector — Codebase Reference

## Overview
Gem Collector is a drag-player collect gems game. Player drags a collector circle around the screen. Gems fall and must be collected before hitting the bottom. Red gems (bad) must be avoided. Spawn rate increases with score. The global namespace is `GemCollector` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `GemCollector`
...
| `best` | All-time best score (from localStorage `gemcollector_best`) |
...
| Key | Content |
|---|---|
| `gemcollector_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'gemcollector_best')` |
