# Pattern Repeat — Codebase Reference

## Overview
Pattern Repeat is a Simon Says 4-zone memory game. 4 colored quadrant zones. A sequence flashes; player must reproduce it by tapping zones in order. Sequence length grows by 1 each successful round. The global namespace is `PatternRepeat` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `PatternRepeat`
...
| `best` | All-time best score (from localStorage `patternrepeat_best`) |
...
| Key | Content |
|---|---|
| `patternrepeat_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'patternrepeat_best')` |
