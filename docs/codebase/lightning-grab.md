# Lightning Grab — Codebase Reference

## Overview
Lightning Grab is a flash-and-choose color reflex game. A color flashes briefly. Then 4 colored buttons appear — player must tap the button that matches the flashed color. Response time window shrinks with score. The global namespace is `LightningGrab` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `LightningGrab`
...
| `best` | All-time best score (from localStorage `lightninggrab_best`) |
...
| Key | Content |
|---|---|
| `lightninggrab_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'lightninggrab_best')` |
