# Cannon Launch — Codebase Reference

## Overview
Cannon Launch is a trajectory physics shooting game. Player aims a cannon at targets using two sequential taps (aim then shoot). Ball follows arc trajectory under gravity. Score based on targets hit within limited shots. The global namespace is `CannonLaunch` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `CannonLaunch`
...
| `best` | All-time best score (from localStorage `cannonlaunch_best`) |
...
| Key | Content |
|---|---|
| `cannonlaunch_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'cannonlaunch_best')` |
