# Gravity Flip — Codebase Reference

## Overview
Gravity Flip is a gravity-flip endless corridor game. Character auto-runs right through corridor gaps. Tap to flip gravity (switch floor/ceiling). Wall pairs spawn with gaps player must navigate. Speed increments every SPEED_STEP score. The global namespace is `GravityFlip` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `GravityFlip`
...
| `best` | All-time best score (from localStorage `gravityflip_best`) |
...
| Key | Content |
|---|---|
| `gravityflip_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'gravityflip_best')` |
