# Neon Path — Codebase Reference

## Overview
Neon Path is a two-rail gap-dodge runner game. Ball rides on two horizontal rails (top and bottom). Gaps appear in each rail. Tap to switch ball between top/bottom rail to avoid falling into gaps. Speed increases with score. The global namespace is `NeonPath` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `NeonPath`
...
| `best` | All-time best score (from localStorage `neonpath_best`) |
...
| Key | Content |
|---|---|
| `neonpath_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'neonpath_best')` |
