# Spike Field — Codebase Reference

## Overview
Spike Field is a physics ball climb game. Ball launched upward by tap. Platforms with spikes on top/bottom at various heights. Ball must land on safe platform tops and bounce up. Higher altitude = more score. The global namespace is `SpikeField` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `SpikeField`
...
| `best` | All-time best score (from localStorage `spikefield_best`) |
...
| Key | Content |
|---|---|
| `spikefield_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'spikefield_best')` |
