# Color Burst — Codebase Reference

## Overview
Color Burst is a color-matching ring arcade game. A ball sits at the center. Colored rings expand outward. Tap when the ball color matches the ring color to pass through; wrong color = life lost. Ball cycles colors on each pass. The global namespace is `ColorBurst` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `ColorBurst`
...
| `best` | All-time best score (from localStorage `colorburst_best`) |
...
| Key | Content |
|---|---|
| `colorburst_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'colorburst_best')` |
