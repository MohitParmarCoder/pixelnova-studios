# Target Blitz — Codebase Reference

## Overview
Target Blitz is a 30-second sweep target game. Concentric rings on screen with a rotating sweep bar. Tap when the bar crosses the target zone (innermost ring = highest points). 30-second timed mode. Bar speed increases. The global namespace is `TargetBlitz` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `TargetBlitz`
...
| `timeLeft` | Player resource (timeLeft) |
| `best` | All-time best score (from localStorage `targetblitz_best`) |
...
| Key | Content |
|---|---|
| `targetblitz_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'targetblitz_best')` |
