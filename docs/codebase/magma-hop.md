# Magma Hop — Codebase Reference

## Overview
Magma Hop is a frogger-style lane hop game. Frog starts at bottom; tap direction to hop across 7 lanes of traffic to the top safe zone. Traffic moves at varying speeds. Score = successful crossings. Lives lost on collision. The global namespace is `MagmaHop` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `MagmaHop`
...
| `best` | All-time best score (from localStorage `magmahop_best`) |
...
| Key | Content |
|---|---|
| `magmahop_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'magmahop_best')` |
