# Color Switch — Codebase Reference

## Overview
Color Switch is a color-matching ring-pass jumper game. Ball jumps upward; colored rotating rings block the path. Ball can only pass through the ring arc whose color matches the ball's current color. Ball color changes on each pass. The global namespace is `ColorSwitch` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `ColorSwitch`
...
| `best` | All-time best score (from localStorage `colorswitch_best`) |
...
| Key | Content |
|---|---|
| `colorswitch_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'colorswitch_best')` |
