# Dino Dash — Codebase Reference

## Overview
Dino Dash is a side-scrolling dinosaur jump game. Auto-running dinosaur; tap to jump over incoming obstacles and under aerial hazards. Double-tap for double jump. Speed increases with score. Invincibility period after each hit. The global namespace is `DinoDash` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `DinoDash`
...
| `best` | All-time best score (from localStorage `dinodash_best`) |
...
| Key | Content |
|---|---|
| `dinodash_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'dinodash_best')` |
