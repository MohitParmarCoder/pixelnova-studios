# Lava Surf — Codebase Reference

## Overview
Lava Surf is a ice curling aim-and-release game. Ice curling mechanic: tap left/right arrows to adjust stone aim angle, then tap RELEASE to launch. Stone slides to bullseye rings scoring 4/6/8/10 pts by ring zone. 6 stones per game. The global namespace is `LavaSurf` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `LavaSurf`
...
| `best` | All-time best score (from localStorage `lavasurf_best`) |
...
| Key | Content |
|---|---|
| `lavasurf_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'lavasurf_best')` |
