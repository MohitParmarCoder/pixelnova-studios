# Missile Evade — Codebase Reference

## Overview
Missile Evade is a missile interception defense game. Missiles target cities from the top. Tap at missile positions to fire interceptor shots. Cities protected by HP. Wave-based escalation. Score = missiles intercepted. The global namespace is `MissileEvade` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `MissileEvade`
...
| `best` | All-time best score (from localStorage `missileevade_best`) |
...
| Key | Content |
|---|---|
| `missileevade_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'missileevade_best')` |
