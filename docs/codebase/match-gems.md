# Match Gems — Codebase Reference

## Overview
Match Gems is a swap-to-match gem puzzle game. 5×5 gem grid. Tap two adjacent gems to swap them. If the swap creates a row/column of 3+ matching gems, they clear and gems fall to fill gaps. 60-second timer. The global namespace is `MatchGems` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `MatchGems`
...
| `best` | All-time best score (from localStorage `matchgems_best`) |
...
| Key | Content |
|---|---|
| `matchgems_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'matchgems_best')` |
