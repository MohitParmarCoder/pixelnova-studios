# Star Grab — Codebase Reference

## Overview
Star Grab is a tap falling stars game. Stars spawn and fall across screen. Tap to collect before they exit. Stars expire at bottom; each expiry costs a life. Spawn rate and speed increase with score. The global namespace is `StarGrab` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `StarGrab`
...
| `best` | All-time best score (from localStorage `stargrab_best`) |
...
| Key | Content |
|---|---|
| `stargrab_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'stargrab_best')` |
