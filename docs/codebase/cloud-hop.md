# Cloud Hop — Codebase Reference

## Overview
Cloud Hop is a hold-to-fly kite navigation game. Player controls a kite that rises while finger is held down and falls when released. Navigate around birds and collect stars. World scrolls horizontally at constant speed. The global namespace is `CloudHop` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `CloudHop`
...
| `best` | All-time best score (from localStorage `cloudhop_best`) |
...
| Key | Content |
|---|---|
| `cloudhop_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'cloudhop_best')` |
