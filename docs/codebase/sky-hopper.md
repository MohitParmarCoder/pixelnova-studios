# Sky Hopper — Codebase Reference

## Overview
Sky Hopper is a Doodle Jump auto-bounce climber game. Character auto-bounces off platforms and climbs upward. Tilt device (or tap left/right) to move horizontally. Reach new platforms; fall below camera = DEAD. Score = height climbed. The global namespace is `SkyHopper` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `SkyHopper`
...
| `best` | All-time best score (from localStorage `skyhopper_best`) |
...
| Key | Content |
|---|---|
| `skyhopper_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'skyhopper_best')` |
