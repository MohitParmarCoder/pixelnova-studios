# Bubble Pop — Codebase Reference

## Overview
Bubble Pop is a bubble shooter game. Fire colored bubbles from the bottom; match 3+ same-color bubbles to pop them. New rows drop every N shots. Game over if bubbles reach the deadline Y. The global namespace is `BubblePop` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `BubblePop`
...
| `best` | All-time best score (from localStorage `bubblepop_best`) |
...
| Key | Content |
|---|---|
| `bubblepop_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'bubblepop_best')` |
