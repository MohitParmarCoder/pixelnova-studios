# Plank Bridge — Codebase Reference

## Overview
Plank Bridge is a hold-to-grow plank game. Hold finger to extend a plank (line rotates to horizontal). Release to drop it. Plank must exactly bridge the gap between platforms. Too short or too long = DEAD. Score = platforms crossed. The global namespace is `PlankBridge` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `PlankBridge`
...
| `best` | All-time best score (from localStorage `plankbridge_best`) |
...
| Key | Content |
|---|---|
| `plankbridge_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'plankbridge_best')` |
