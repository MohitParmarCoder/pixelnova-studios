# Dodge Rush — Codebase Reference

## Overview
Dodge Rush is a survive falling shapes game. Player at bottom of screen moves left/right to dodge falling geometric shapes (triangles, squares, diamonds). Power-ups grant temporary invincibility. Survival time = score. The global namespace is `DodgeRush` and the entire game logic is contained in `js/game.js`.

## File: js/game.js
- **Global namespace:** `DodgeRush`
...
| `invuln` | Player resource (invuln) |
| `best` | All-time best score (from localStorage `dodgerush_best`) |
...
| Key | Content |
|---|---|
| `dodgerush_best` | Highest score achieved |
...
| DEAD screen | `AdManager.offerDoubleScore(score, 'dodgerush_best')` |
