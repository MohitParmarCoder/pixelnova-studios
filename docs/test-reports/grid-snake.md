...
## Syntax Check
```bash
node --check grid-snake/js/game.js
...
| Best score persisted to `gridsnake_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — movement is tick-based so dt mainly affects animation; clamping prevents double-steps.
