...
## Syntax Check
```bash
node --check peg-drop/js/game.js
...
| Best score persisted to `pegdrop_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — ball physics uses dt; clamping prevents tunneling through pegs. Stuck balls fix in PR #22.
