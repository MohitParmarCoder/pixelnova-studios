...
## Syntax Check
```bash
node --check fruit-catcher/js/game.js
...
| Best score persisted to `fruitcatcher_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — multi-life drain fix: max 1 life lost per frame regardless of fruit count. PR #22.
