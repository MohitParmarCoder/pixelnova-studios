...
## Syntax Check
```bash
node --check hex-flip/js/game.js
...
| Best score persisted to `hexflip_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — timer countdown uses dt; clamping prevents large single-frame jumps.
