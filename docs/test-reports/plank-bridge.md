...
## Syntax Check
```bash
node --check plank-bridge/js/game.js
...
| Best score persisted to `plankbridge_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — plank growth rate uses dt; clamping prevents overshooting on slow frames.
