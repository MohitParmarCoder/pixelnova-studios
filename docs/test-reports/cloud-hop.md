...
## Syntax Check
```bash
node --check cloud-hop/js/game.js
...
| Best score persisted to `cloudhop_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — prevents kite velocity from exceeding ceiling/floor bounds in a single frame.
