...
## Syntax Check
```bash
node --check number-merge/js/game.js
...
| Best score persisted to `numbermerge_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — movement is tile-based so clamping only affects particle animations.
