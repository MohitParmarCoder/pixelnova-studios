...
## Syntax Check
```bash
node --check coin-catch/js/game.js
...
| Best score persisted to `coincatch_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — prevents coins from teleporting past catch zone. Grace period verified working post-fix.
