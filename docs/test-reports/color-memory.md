...
## Syntax Check
```bash
node --check color-memory/js/game.js
...
| Best score persisted to `colormemory_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — sequence playback timing unaffected; only animation delta uses dt.
