...
## Syntax Check
```bash
node --check number-order/js/game.js
...
| Best score persisted to `numberorder_best` | PASS |
...
## dt Clamp Fix (PR #22)
dt clamped to 0.05s — timer uses dt; clamping prevents large single-frame time jumps.
