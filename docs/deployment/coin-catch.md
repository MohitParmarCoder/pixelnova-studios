# Coin Catch — Deployment Guide

## Overview
Coin Catch is a zero-dependency HTML5 canvas game (falling coin collection game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory coin-catch
# Open http://localhost:8080
node --check coin-catch/js/game.js   # syntax check
```
...
- [ ] localStorage key `coincatch_best` persists best score across refreshes
