# Hex Flip — Deployment Guide

## Overview
Hex Flip is a zero-dependency HTML5 canvas game (tap hexagons to match colors puzzle). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory hex-flip
# Open http://localhost:8080
node --check hex-flip/js/game.js   # syntax check
```
...
- [ ] localStorage key `hexflip_best` persists best score across refreshes
