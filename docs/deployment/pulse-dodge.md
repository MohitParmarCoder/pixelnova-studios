# Pulse Dodge — Deployment Guide

## Overview
Pulse Dodge is a zero-dependency HTML5 canvas game (orbit around center dodge ring waves game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory pulse-dodge
# Open http://localhost:8080
node --check pulse-dodge/js/game.js   # syntax check
```
...
- [ ] localStorage key `pulsedodge_best` persists best score across refreshes
