# Card Pairs — Deployment Guide

## Overview
Card Pairs is a zero-dependency HTML5 canvas game (memory card-matching flip game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory card-pairs
# Open http://localhost:8080
node --check card-pairs/js/game.js   # syntax check
```
...
- [ ] localStorage key `cardpairs_best` persists best score across refreshes
