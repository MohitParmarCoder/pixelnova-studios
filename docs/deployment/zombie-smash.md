# Zombie Smash — Deployment Guide

## Overview
Zombie Smash is a zero-dependency HTML5 canvas game (tap zombies before they reach the center). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
zombie-smash/
  ...
  js/
    ...
    game.js         # ZombieSmash game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory zombie-smash
# Open http://localhost:8080
node --check zombie-smash/js/game.js   # syntax check
```
...
- [ ] localStorage key `zombiesmash_best` persists best score across refreshes
