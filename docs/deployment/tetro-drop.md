# Tetro Drop — Deployment Guide

## Overview
Tetro Drop is a zero-dependency HTML5 canvas game (rotating board ball-into-hole game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory tetro-drop
# Open http://localhost:8080
node --check tetro-drop/js/game.js   # syntax check
```
...
- [ ] localStorage key `tetrodrop_best` persists best score across refreshes
