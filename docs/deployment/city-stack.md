# City Stack — Deployment Guide

## Overview
City Stack is a zero-dependency HTML5 canvas game (moving-block city building stacker). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory city-stack
# Open http://localhost:8080
node --check city-stack/js/game.js   # syntax check
```
...
- [ ] localStorage key `citystack_best` persists best score across refreshes
