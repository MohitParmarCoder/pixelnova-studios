# Cloud Hop — Deployment Guide

## Overview
Cloud Hop is a zero-dependency HTML5 canvas game (hold-to-fly kite navigation game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory cloud-hop
# Open http://localhost:8080
node --check cloud-hop/js/game.js   # syntax check
```
...
- [ ] localStorage key `cloudhop_best` persists best score across refreshes
