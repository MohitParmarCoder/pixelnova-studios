# Stack Tower — Deployment Guide

## Overview
Stack Tower is a zero-dependency HTML5 canvas game (precision neon block stacking game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory stack-tower
# Open http://localhost:8080
node --check stack-tower/js/game.js   # syntax check
```
...
- [ ] localStorage key `stacktower_best` persists best score across refreshes
