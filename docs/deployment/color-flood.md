# Color Flood — Deployment Guide

## Overview
Color Flood is a zero-dependency HTML5 canvas game (15-puzzle slide tile game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory color-flood
# Open http://localhost:8080
node --check color-flood/js/game.js   # syntax check
```
...
- [ ] localStorage key `colorflood_best` persists best score across refreshes
