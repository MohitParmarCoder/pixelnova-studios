# Color Memory — Deployment Guide

## Overview
Color Memory is a zero-dependency HTML5 canvas game (piano key melody memory game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
color-memory/
  ...
  js/
    ...
    game.js         # ColorMemory game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory color-memory
# Open http://localhost:8080
node --check color-memory/js/game.js   # syntax check
```
...
- [ ] localStorage key `colormemory_best` persists best score across refreshes
