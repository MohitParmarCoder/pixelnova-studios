# Lava Surf — Deployment Guide

## Overview
Lava Surf is a zero-dependency HTML5 canvas game (ice curling aim-and-release game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory lava-surf
# Open http://localhost:8080
node --check lava-surf/js/game.js   # syntax check
```
...
- [ ] localStorage key `lavasurf_best` persists best score across refreshes
