# Pattern Repeat — Deployment Guide

## Overview
Pattern Repeat is a zero-dependency HTML5 canvas game (Simon-Says 4-zone memory game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
pattern-repeat/
  ...
  js/
    ...
    game.js         # PatternRepeat game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory pattern-repeat
# Open http://localhost:8080
node --check pattern-repeat/js/game.js   # syntax check
```
...
- [ ] localStorage key `patternrepeat_best` persists best score across refreshes
