# Match Gems — Deployment Guide

## Overview
Match Gems is a zero-dependency HTML5 canvas game (swap-to-match gem puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory match-gems
# Open http://localhost:8080
node --check match-gems/js/game.js   # syntax check
```
...
- [ ] localStorage key `matchgems_best` persists best score across refreshes
