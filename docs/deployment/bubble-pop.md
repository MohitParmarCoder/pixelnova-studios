# Bubble Pop — Deployment Guide

## Overview
Bubble Pop is a zero-dependency HTML5 canvas game (bubble shooter match game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory bubble-pop
# Open http://localhost:8080
node --check bubble-pop/js/game.js   # syntax check
```
...
- [ ] localStorage key `bubblepop_best` persists best score across refreshes
