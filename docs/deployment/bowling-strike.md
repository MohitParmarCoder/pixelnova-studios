# Bowling Strike — Deployment Guide

## Overview
Bowling Strike is a zero-dependency HTML5 canvas game (touch-controlled bowling physics game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
bowling-strike/
  ...
  js/
    ...
    game.js         # BowlingStrike game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory bowling-strike
# Open http://localhost:8080
node --check bowling-strike/js/game.js   # syntax check
```
...
- [ ] localStorage key `bowlingstrike_best` persists best score across refreshes
