# Water Flow — Deployment Guide

## Overview
Water Flow is a zero-dependency HTML5 canvas game (pipe-rotation flow puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory water-flow
# Open http://localhost:8080
node --check water-flow/js/game.js   # syntax check
```
...
- [ ] localStorage key `waterflow_best` persists best score across refreshes
