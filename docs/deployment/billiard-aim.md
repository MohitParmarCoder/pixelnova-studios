# Billiard Aim — Deployment Guide

## Overview
Billiard Aim is a zero-dependency HTML5 canvas game (billiards/pool aim-and-shoot game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
billiard-aim/
  ...
  js/
    ...
    game.js         # BilliardAim game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory billiard-aim
# Open http://localhost:8080
node --check billiard-aim/js/game.js   # syntax check
```
...
- [ ] localStorage key `billiardaim_best` persists best score across refreshes
