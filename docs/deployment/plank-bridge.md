# Plank Bridge — Deployment Guide

## Overview
Plank Bridge is a zero-dependency HTML5 canvas game (hold-to-grow plank bridge game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
plank-bridge/
  ...
  js/
    ...
    game.js         # PlankBridge game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory plank-bridge
# Open http://localhost:8080
node --check plank-bridge/js/game.js   # syntax check
```
...
- [ ] localStorage key `plankbridge_best` persists best score across refreshes
