# Paint Fill — Deployment Guide

## Overview
Paint Fill is a zero-dependency HTML5 canvas game (flood-fill color puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory paint-fill
# Open http://localhost:8080
node --check paint-fill/js/game.js   # syntax check
```
...
- [ ] localStorage key `paintfill_best` persists best score across refreshes
