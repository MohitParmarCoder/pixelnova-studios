# Spike Field — Deployment Guide

## Overview
Spike Field is a zero-dependency HTML5 canvas game (physics ball climb spike-dodge game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory spike-field
# Open http://localhost:8080
node --check spike-field/js/game.js   # syntax check
```
...
- [ ] localStorage key `spikefield_best` persists best score across refreshes
