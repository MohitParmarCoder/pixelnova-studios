# Dino Dash — Deployment Guide

## Overview
Dino Dash is a zero-dependency HTML5 canvas game (side-scrolling dinosaur jump game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory dino-dash
# Open http://localhost:8080
node --check dino-dash/js/game.js   # syntax check
```
...
- [ ] localStorage key `dinodash_best` persists best score across refreshes
