# Sequence Game — Deployment Guide

## Overview
Sequence Game is a zero-dependency HTML5 canvas game (Mastermind code-breaker puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
sequence-game/
  ...
  js/
    ...
    game.js         # SequenceGame game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory sequence-game
# Open http://localhost:8080
node --check sequence-game/js/game.js   # syntax check
```
...
- [ ] localStorage key `sequencegame_best` persists best score across refreshes
