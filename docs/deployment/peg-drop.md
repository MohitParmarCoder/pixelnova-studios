# Peg Drop — Deployment Guide

## Overview
Peg Drop is a zero-dependency HTML5 canvas game (Plinko peg-board ball drop game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory peg-drop
# Open http://localhost:8080
node --check peg-drop/js/game.js   # syntax check
```
...
- [ ] localStorage key `pegdrop_best` persists best score across refreshes
