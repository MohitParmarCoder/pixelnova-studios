# Slingshot Aim — Deployment Guide

## Overview
Slingshot Aim is a zero-dependency HTML5 canvas game (tap-aim slingshot physics game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
slingshot-aim/
  ...
  js/
    ...
    game.js         # SlingshotAim game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory slingshot-aim
# Open http://localhost:8080
node --check slingshot-aim/js/game.js   # syntax check
```
...
- [ ] localStorage key `slingshotaim_best` persists best score across refreshes
