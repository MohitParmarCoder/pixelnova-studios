# Color Tap — Deployment Guide

## Overview
Color Tap is a zero-dependency HTML5 canvas game (stroop color recognition tap game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory color-tap
# Open http://localhost:8080
node --check color-tap/js/game.js   # syntax check
```
...
- [ ] localStorage key `colortap_best` persists best score across refreshes
