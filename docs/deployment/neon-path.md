# Neon Path — Deployment Guide

## Overview
Neon Path is a zero-dependency HTML5 canvas game (two-rail gap-dodge runner). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory neon-path
# Open http://localhost:8080
node --check neon-path/js/game.js   # syntax check
```
...
- [ ] localStorage key `neonpath_best` persists best score across refreshes
