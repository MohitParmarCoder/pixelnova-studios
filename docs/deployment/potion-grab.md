# Potion Grab — Deployment Guide

## Overview
Potion Grab is a zero-dependency HTML5 canvas game (color-mix slider puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory potion-grab
# Open http://localhost:8080
node --check potion-grab/js/game.js   # syntax check
```
...
- [ ] localStorage key `potiongrab_best` persists best score across refreshes
