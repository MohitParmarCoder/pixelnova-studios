# Tap Blast — Deployment Guide

## Overview
Tap Blast is a zero-dependency HTML5 canvas game (timed circle tap before shrink game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory tap-blast
# Open http://localhost:8080
node --check tap-blast/js/game.js   # syntax check
```
...
- [ ] localStorage key `tapblast_best` persists best score across refreshes
