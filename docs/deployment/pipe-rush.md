# Pipe Rush — Deployment Guide

## Overview
Pipe Rush is a zero-dependency HTML5 canvas game (Flappy-meets-color-switch orb game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory pipe-rush
# Open http://localhost:8080
node --check pipe-rush/js/game.js   # syntax check
```
...
- [ ] localStorage key `piperush_best` persists best score across refreshes
