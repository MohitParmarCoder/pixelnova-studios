# Dodge Rush — Deployment Guide

## Overview
Dodge Rush is a zero-dependency HTML5 canvas game (survive falling shapes dodge game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory dodge-rush
# Open http://localhost:8080
node --check dodge-rush/js/game.js   # syntax check
```
...
- [ ] localStorage key `dodgerush_best` persists best score across refreshes
