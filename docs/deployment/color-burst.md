# Color Burst — Deployment Guide

## Overview
Color Burst is a zero-dependency HTML5 canvas game (color-matching ball-and-ring arcade game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory color-burst
# Open http://localhost:8080
node --check color-burst/js/game.js   # syntax check
```
...
- [ ] localStorage key `colorburst_best` persists best score across refreshes
