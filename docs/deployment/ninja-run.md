# Ninja Run — Deployment Guide

## Overview
Ninja Run is a zero-dependency HTML5 canvas game (rope-swing ninja climbing game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory ninja-run
# Open http://localhost:8080
node --check ninja-run/js/game.js   # syntax check
```
...
- [ ] localStorage key `ninjarun_best` persists best score across refreshes
