# Pinball Lite — Deployment Guide

## Overview
Pinball Lite is a zero-dependency HTML5 canvas game (pinball physics arcade game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
pinball-lite/
  ...
  js/
    ...
    game.js         # PinballLite game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory pinball-lite
# Open http://localhost:8080
node --check pinball-lite/js/game.js   # syntax check
```
...
- [ ] localStorage key `pinballlite_best` persists best score across refreshes
