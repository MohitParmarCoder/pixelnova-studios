# Color Switch — Deployment Guide

## Overview
Color Switch is a zero-dependency HTML5 canvas game (color-matching ring-pass jump game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
color-switch/
  ...
  js/
    ...
    game.js         # ColorSwitch game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory color-switch
# Open http://localhost:8080
node --check color-switch/js/game.js   # syntax check
```
...
- [ ] localStorage key `colorswitch_best` persists best score across refreshes
