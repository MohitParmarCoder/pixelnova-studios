# Grid Snake — Deployment Guide

## Overview
Grid Snake is a zero-dependency HTML5 canvas game (snake game with numeric food values). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory grid-snake
# Open http://localhost:8080
node --check grid-snake/js/game.js   # syntax check
```
...
- [ ] localStorage key `gridsnake_best` persists best score across refreshes
