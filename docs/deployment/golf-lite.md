# Golf Lite — Deployment Guide

## Overview
Golf Lite is a zero-dependency HTML5 canvas game (mini-golf tap-aim-shoot game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory golf-lite
# Open http://localhost:8080
node --check golf-lite/js/game.js   # syntax check
```
...
- [ ] localStorage key `golflite_best` persists best score across refreshes
