# Pixel Trace — Deployment Guide

## Overview
Pixel Trace is a zero-dependency HTML5 canvas game (pixel art tracing puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory pixel-trace
# Open http://localhost:8080
node --check pixel-trace/js/game.js   # syntax check
```
...
- [ ] localStorage key `pixeltrace_best` persists best score across refreshes
