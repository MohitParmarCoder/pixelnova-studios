# Cave Runner — Deployment Guide

## Overview
Cave Runner is a zero-dependency HTML5 canvas game (grid-based mining dungeon game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory cave-runner
# Open http://localhost:8080
node --check cave-runner/js/game.js   # syntax check
```
...
- [ ] localStorage key `caverunner_best` persists best score across refreshes
