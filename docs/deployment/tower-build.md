# Tower Build — Deployment Guide

## Overview
Tower Build is a zero-dependency HTML5 canvas game (pastel precision block stacker). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory tower-build
# Open http://localhost:8080
node --check tower-build/js/game.js   # syntax check
```
...
- [ ] localStorage key `towerbuild_best` persists best score across refreshes
