# Block Blast — Deployment Guide

## Overview
Block Blast is a zero-dependency HTML5 canvas game (block-dropping puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory block-blast
# Open http://localhost:8080
node --check block-blast/js/game.js   # syntax check
```
...
- [ ] localStorage key `blockblast_best` persists best score across refreshes
