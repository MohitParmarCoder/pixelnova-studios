# Sky Hopper — Deployment Guide

## Overview
Sky Hopper is a zero-dependency HTML5 canvas game (Doodle Jump auto-bounce climber). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory sky-hopper
# Open http://localhost:8080
node --check sky-hopper/js/game.js   # syntax check
```
...
- [ ] localStorage key `skyhopper_best` persists best score across refreshes
