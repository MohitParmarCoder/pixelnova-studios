# Gravity Flip — Deployment Guide

## Overview
Gravity Flip is a zero-dependency HTML5 canvas game (gravity-flip endless corridor runner). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
gravity-flip/
  ...
  js/
    ...
    game.js         # GravityFlip game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory gravity-flip
# Open http://localhost:8080
node --check gravity-flip/js/game.js   # syntax check
```
...
- [ ] localStorage key `gravityflip_best` persists best score across refreshes
