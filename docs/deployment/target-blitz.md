# Target Blitz — Deployment Guide

## Overview
Target Blitz is a zero-dependency HTML5 canvas game (30-second sweep-target scoring game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
target-blitz/
  ...
  js/
    ...
    game.js         # TargetBlitz game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory target-blitz
# Open http://localhost:8080
node --check target-blitz/js/game.js   # syntax check
```
...
- [ ] localStorage key `targetblitz_best` persists best score across refreshes
