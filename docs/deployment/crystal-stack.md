# Crystal Stack — Deployment Guide

## Overview
Crystal Stack is a zero-dependency HTML5 canvas game (tube ball color-sort puzzle). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
crystal-stack/
  ...
  js/
    ...
    game.js         # CrystalStack game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory crystal-stack
# Open http://localhost:8080
node --check crystal-stack/js/game.js   # syntax check
```
...
- [ ] localStorage key `crystalstack_best` persists best score across refreshes
