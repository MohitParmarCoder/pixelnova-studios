# Number Order — Deployment Guide

## Overview
Number Order is a zero-dependency HTML5 canvas game (tap numbers in ascending order game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
number-order/
  ...
  js/
    ...
    game.js         # NumberOrder game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory number-order
# Open http://localhost:8080
node --check number-order/js/game.js   # syntax check
```
...
- [ ] localStorage key `numberorder_best` persists best score across refreshes
