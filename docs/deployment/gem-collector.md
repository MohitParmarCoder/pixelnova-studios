# Gem Collector — Deployment Guide

## Overview
Gem Collector is a zero-dependency HTML5 canvas game (drag-to-collect gem arcade game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
gem-collector/
  ...
  js/
    ...
    game.js         # GemCollector game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory gem-collector
# Open http://localhost:8080
node --check gem-collector/js/game.js   # syntax check
```
...
- [ ] localStorage key `gemcollector_best` persists best score across refreshes
