# Dot Link — Deployment Guide

## Overview
Dot Link is a zero-dependency HTML5 canvas game (connect-the-dots flow puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory dot-link
# Open http://localhost:8080
node --check dot-link/js/game.js   # syntax check
```
...
- [ ] localStorage key `dotlink_best` persists best score across refreshes
