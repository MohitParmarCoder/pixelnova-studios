# Wall Rise — Deployment Guide

## Overview
Wall Rise is a zero-dependency HTML5 canvas game (column brick-rising tap-to-clear game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory wall-rise
# Open http://localhost:8080
node --check wall-rise/js/game.js   # syntax check
```
...
- [ ] localStorage key `wallrise_best` persists best score across refreshes
