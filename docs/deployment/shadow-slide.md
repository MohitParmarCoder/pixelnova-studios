# Shadow Slide — Deployment Guide

## Overview
Shadow Slide is a zero-dependency HTML5 canvas game (neon platform jump and slide game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
shadow-slide/
  ...
  js/
    ...
    game.js         # ShadowSlide game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory shadow-slide
# Open http://localhost:8080
node --check shadow-slide/js/game.js   # syntax check
```
...
- [ ] localStorage key `shadowslide_best` persists best score across refreshes
