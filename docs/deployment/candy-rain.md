# Candy Rain — Deployment Guide

## Overview
Candy Rain is a zero-dependency HTML5 canvas game (number-pair matching tap game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory candy-rain
# Open http://localhost:8080
node --check candy-rain/js/game.js   # syntax check
```
...
- [ ] localStorage key `candyrain_best` persists best score across refreshes
