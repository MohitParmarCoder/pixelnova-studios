# Shape Recall — Deployment Guide

## Overview
Shape Recall is a zero-dependency HTML5 canvas game (tangram shape-placement puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
shape-recall/
  ...
  js/
    ...
    game.js         # ShapeRecall game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory shape-recall
# Open http://localhost:8080
node --check shape-recall/js/game.js   # syntax check
```
...
- [ ] localStorage key `shaperecall_best` persists best score across refreshes
