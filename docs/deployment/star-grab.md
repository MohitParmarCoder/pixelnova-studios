# Star Grab — Deployment Guide

## Overview
Star Grab is a zero-dependency HTML5 canvas game (tap falling stars before they vanish). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory star-grab
# Open http://localhost:8080
node --check star-grab/js/game.js   # syntax check
```
...
- [ ] localStorage key `stargrab_best` persists best score across refreshes
