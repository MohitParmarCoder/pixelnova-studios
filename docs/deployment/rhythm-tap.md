# Rhythm Tap — Deployment Guide

## Overview
Rhythm Tap is a zero-dependency HTML5 canvas game (falling circles rhythm tap game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory rhythm-tap
# Open http://localhost:8080
node --check rhythm-tap/js/game.js   # syntax check
```
...
- [ ] localStorage key `rhythmtap_best` persists best score across refreshes
