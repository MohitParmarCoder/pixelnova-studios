# Speed Click — Deployment Guide

## Overview
Speed Click is a zero-dependency HTML5 canvas game (fast-tap shrinking dot reaction game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory speed-click
# Open http://localhost:8080
node --check speed-click/js/game.js   # syntax check
```
...
- [ ] localStorage key `speedclick_best` persists best score across refreshes
