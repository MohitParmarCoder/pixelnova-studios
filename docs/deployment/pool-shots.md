# Pool Shots — Deployment Guide

## Overview
Pool Shots is a zero-dependency HTML5 canvas game (disc sliding zone-score game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory pool-shots
# Open http://localhost:8080
node --check pool-shots/js/game.js   # syntax check
```
...
- [ ] localStorage key `poolshots_best` persists best score across refreshes
