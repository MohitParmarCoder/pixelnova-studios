# Magma Hop — Deployment Guide

## Overview
Magma Hop is a zero-dependency HTML5 canvas game (frogger-style lane-crossing hop game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory magma-hop
# Open http://localhost:8080
node --check magma-hop/js/game.js   # syntax check
```
...
- [ ] localStorage key `magmahop_best` persists best score across refreshes
