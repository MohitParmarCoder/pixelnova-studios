# Traffic Rush — Deployment Guide

## Overview
Traffic Rush is a zero-dependency HTML5 canvas game (frogger-style 7-lane traffic dodge). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
traffic-rush/
  ...
  js/
    ...
    game.js         # TrafficRush game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory traffic-rush
# Open http://localhost:8080
node --check traffic-rush/js/game.js   # syntax check
```
...
- [ ] localStorage key `trafficrush_best` persists best score across refreshes
