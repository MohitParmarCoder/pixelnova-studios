# Cannon Launch — Deployment Guide

## Overview
Cannon Launch is a zero-dependency HTML5 canvas game (trajectory-physics cannon aiming game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
cannon-launch/
  ...
  js/
    ...
    game.js         # CannonLaunch game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory cannon-launch
# Open http://localhost:8080
node --check cannon-launch/js/game.js   # syntax check
```
...
- [ ] localStorage key `cannonlaunch_best` persists best score across refreshes
