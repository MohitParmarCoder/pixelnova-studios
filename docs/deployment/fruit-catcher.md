# Fruit Catcher — Deployment Guide

## Overview
Fruit Catcher is a zero-dependency HTML5 canvas game (slash-fruits bomb-dodge arcade game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
fruit-catcher/
  ...
  js/
    ...
    game.js         # FruitCatcher game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory fruit-catcher
# Open http://localhost:8080
node --check fruit-catcher/js/game.js   # syntax check
```
...
- [ ] localStorage key `fruitcatcher_best` persists best score across refreshes
