# Balance Beam — Deployment Guide

## Overview
Balance Beam is a zero-dependency HTML5 canvas game (physics weight-balancing puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
balance-beam/
  ...
  js/
    ...
    game.js         # BalanceBeam game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory balance-beam
# Open http://localhost:8080
node --check balance-beam/js/game.js   # syntax check
```
...
- [ ] localStorage key `balancebeam_best` persists best score across refreshes
