# Flash Tap — Deployment Guide

## Overview
Flash Tap is a zero-dependency HTML5 canvas game (memory grid flash-and-tap game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory flash-tap
# Open http://localhost:8080
node --check flash-tap/js/game.js   # syntax check
```
...
- [ ] localStorage key `flashtap_best` persists best score across refreshes
