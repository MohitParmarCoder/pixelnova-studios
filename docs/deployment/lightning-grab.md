# Lightning Grab — Deployment Guide

## Overview
Lightning Grab is a zero-dependency HTML5 canvas game (flash-and-choose color reflex game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
lightning-grab/
  ...
  js/
    ...
    game.js         # LightningGrab game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory lightning-grab
# Open http://localhost:8080
node --check lightning-grab/js/game.js   # syntax check
```
...
- [ ] localStorage key `lightninggrab_best` persists best score across refreshes
