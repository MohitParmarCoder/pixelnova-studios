# Word Blitz — Deployment Guide

## Overview
Word Blitz is a zero-dependency HTML5 canvas game (tap letters to spell the target word). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory word-blitz
# Open http://localhost:8080
node --check word-blitz/js/game.js   # syntax check
```
...
- [ ] localStorage key `wordblitz_best` persists best score across refreshes
