# Mirror Tap — Deployment Guide

## Overview
Mirror Tap is a zero-dependency HTML5 canvas game (mirror gesture matching game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory mirror-tap
# Open http://localhost:8080
node --check mirror-tap/js/game.js   # syntax check
```
...
- [ ] localStorage key `mirrortap_best` persists best score across refreshes
