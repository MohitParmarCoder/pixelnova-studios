# Missile Evade — Deployment Guide

## Overview
Missile Evade is a zero-dependency HTML5 canvas game (missile interception defense game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
missile-evade/
  ...
  js/
    ...
    game.js         # MissileEvade game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory missile-evade
# Open http://localhost:8080
node --check missile-evade/js/game.js   # syntax check
```
...
- [ ] localStorage key `missileevade_best` persists best score across refreshes
