# Egg Drop — Deployment Guide

## Overview
Egg Drop is a zero-dependency HTML5 canvas game (catch-items recipe collection game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory egg-drop
# Open http://localhost:8080
node --check egg-drop/js/game.js   # syntax check
```
...
- [ ] localStorage key `eggdrop_best` persists best score across refreshes
