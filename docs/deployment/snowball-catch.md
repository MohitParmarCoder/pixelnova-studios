# Snowball Catch — Deployment Guide

## Overview
Snowball Catch is a zero-dependency HTML5 canvas game (fishing hook catch-fish game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
snowball-catch/
  ...
  js/
    ...
    game.js         # SnowballCatch game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory snowball-catch
# Open http://localhost:8080
node --check snowball-catch/js/game.js   # syntax check
```
...
- [ ] localStorage key `snowballcatch_best` persists best score across refreshes
