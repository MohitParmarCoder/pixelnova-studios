# Rail Slide — Deployment Guide

## Overview
Rail Slide is a zero-dependency HTML5 canvas game (grind-rail obstacle dodge game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory rail-slide
# Open http://localhost:8080
node --check rail-slide/js/game.js   # syntax check
```
...
- [ ] localStorage key `railslide_best` persists best score across refreshes
