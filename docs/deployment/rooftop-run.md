# Rooftop Run — Deployment Guide

## Overview
Rooftop Run is a zero-dependency HTML5 canvas game (timing-jump rooftop platformer). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory rooftop-run
# Open http://localhost:8080
node --check rooftop-run/js/game.js   # syntax check
```
...
- [ ] localStorage key `rooftoprun_best` persists best score across refreshes
