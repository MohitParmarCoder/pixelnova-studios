# Block Drop — Deployment Guide

## Overview
Block Drop is a zero-dependency HTML5 canvas game (block stacking/dropping game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory block-drop
# Open http://localhost:8080
node --check block-drop/js/game.js   # syntax check
```
...
- [ ] localStorage key `blockdrop_best` persists best score across refreshes
