# Spot It — Deployment Guide

## Overview
Spot It is a zero-dependency HTML5 canvas game (find the odd symbol in the grid game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory spot-it
# Open http://localhost:8080
node --check spot-it/js/game.js   # syntax check
```
...
- [ ] localStorage key `spotit_best` persists best score across refreshes
