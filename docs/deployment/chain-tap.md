# Chain Tap — Deployment Guide

## Overview
Chain Tap is a zero-dependency HTML5 canvas game (color-chain tap sequence game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory chain-tap
# Open http://localhost:8080
node --check chain-tap/js/game.js   # syntax check
```
...
- [ ] localStorage key `chaintap_best` persists best score across refreshes
