# Symbol Hunt — Deployment Guide

## Overview
Symbol Hunt is a zero-dependency HTML5 canvas game (find the target symbol in grid game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory symbol-hunt
# Open http://localhost:8080
node --check symbol-hunt/js/game.js   # syntax check
```
...
- [ ] localStorage key `symbolhunt_best` persists best score across refreshes
