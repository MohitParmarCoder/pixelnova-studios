# Orbit Tap — Deployment Guide

## Overview
Orbit Tap is a zero-dependency HTML5 canvas game (orbital planet tap-to-destroy game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory orbit-tap
# Open http://localhost:8080
node --check orbit-tap/js/game.js   # syntax check
```
...
- [ ] localStorage key `orbittap_best` persists best score across refreshes
