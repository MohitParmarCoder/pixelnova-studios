# Number Merge — Deployment Guide

## Overview
Number Merge is a zero-dependency HTML5 canvas game (2048-style snake number merge game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
number-merge/
  ...
  js/
    ...
    game.js         # NumberMerge game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory number-merge
# Open http://localhost:8080
node --check number-merge/js/game.js   # syntax check
```
...
- [ ] localStorage key `numbermerge_best` persists best score across refreshes
