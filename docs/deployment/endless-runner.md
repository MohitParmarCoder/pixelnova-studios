# Endless Runner — Deployment Guide

## Overview
Endless Runner is a zero-dependency HTML5 canvas game (auto-running jump obstacle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
endless-runner/
  ...
  js/
    ...
    game.js         # EndlessRunner game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory endless-runner
# Open http://localhost:8080
node --check endless-runner/js/game.js   # syntax check
```
...
- [ ] localStorage key `endlessrunner_best` persists best score across refreshes
