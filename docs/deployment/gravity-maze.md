# Gravity Maze — Deployment Guide

## Overview
Gravity Maze is a zero-dependency HTML5 canvas game (flip-gravity ball maze puzzle). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
gravity-maze/
  ...
  js/
    ...
    game.js         # GravityMaze game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory gravity-maze
# Open http://localhost:8080
node --check gravity-maze/js/game.js   # syntax check
```
...
- [ ] localStorage key `gravitymaze_best` persists best score across refreshes
