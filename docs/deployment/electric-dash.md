# Electric Dash — Deployment Guide

## Overview
Electric Dash is a zero-dependency HTML5 canvas game (rotate-wires circuit puzzle game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
electric-dash/
  ...
  js/
    ...
    game.js         # ElectricDash game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory electric-dash
# Open http://localhost:8080
node --check electric-dash/js/game.js   # syntax check
```
...
- [ ] localStorage key `electricdash_best` persists best score across refreshes
