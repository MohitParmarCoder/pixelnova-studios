# Fireball Run — Deployment Guide

## Overview
Fireball Run is a zero-dependency HTML5 canvas game (castle defense arrow shooting game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
fireball-run/
  ...
  js/
    ...
    game.js         # FireballRun game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory fireball-run
# Open http://localhost:8080
node --check fireball-run/js/game.js   # syntax check
```
...
- [ ] localStorage key `fireballrun_best` persists best score across refreshes
