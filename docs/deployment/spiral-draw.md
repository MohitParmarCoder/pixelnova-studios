# Spiral Draw — Deployment Guide

## Overview
Spiral Draw is a zero-dependency HTML5 canvas game (tap dots in spiral order game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory spiral-draw
# Open http://localhost:8080
node --check spiral-draw/js/game.js   # syntax check
```
...
- [ ] localStorage key `spiraldraw_best` persists best score across refreshes
