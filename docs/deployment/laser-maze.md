# Laser Maze — Deployment Guide

## Overview
Laser Maze is a zero-dependency HTML5 canvas game (rotate mirrors to guide laser beam puzzle). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory laser-maze
# Open http://localhost:8080
node --check laser-maze/js/game.js   # syntax check
```
...
- [ ] localStorage key `lasermaze_best` persists best score across refreshes
