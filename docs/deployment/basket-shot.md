# Basket Shot — Deployment Guide

## Overview
Basket Shot is a zero-dependency HTML5 canvas game (basketball shooting arcade game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory basket-shot
# Open http://localhost:8080
node --check basket-shot/js/game.js   # syntax check
```
...
- [ ] localStorage key `basketshot_best` persists best score across refreshes
