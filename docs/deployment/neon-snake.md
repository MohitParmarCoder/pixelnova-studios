# Neon Snake — Deployment Guide

## Overview
Neon Snake is a zero-dependency HTML5 canvas game (classic neon snake eat-and-grow game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory neon-snake
# Open http://localhost:8080
node --check neon-snake/js/game.js   # syntax check
```
...
- [ ] localStorage key `neonsnake_best` persists best score across refreshes
