# Tile Tap — Deployment Guide

## Overview
Tile Tap is a zero-dependency HTML5 canvas game (piano tiles four-lane tap game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory tile-tap
# Open http://localhost:8080
node --check tile-tap/js/game.js   # syntax check
```
...
- [ ] localStorage key `tiletap_best` persists best score across refreshes
