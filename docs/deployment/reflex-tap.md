# Reflex Tap — Deployment Guide

## Overview
Reflex Tap is a zero-dependency HTML5 canvas game (spinning ring hit-zone reflex game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory reflex-tap
# Open http://localhost:8080
node --check reflex-tap/js/game.js   # syntax check
```
...
- [ ] localStorage key `reflextap_best` persists best score across refreshes
