# Orb Collector — Deployment Guide

## Overview
Orb Collector is a zero-dependency HTML5 canvas game (drag-player-to-collect orbs game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
orb-collector/
  ...
  js/
    ...
    game.js         # OrbCollector game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory orb-collector
# Open http://localhost:8080
node --check orb-collector/js/game.js   # syntax check
```
...
- [ ] localStorage key `orbcollector_best` persists best score across refreshes
