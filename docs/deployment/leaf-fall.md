# Leaf Fall — Deployment Guide

## Overview
Leaf Fall is a zero-dependency HTML5 canvas game (tap moving bugs falling game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory leaf-fall
# Open http://localhost:8080
node --check leaf-fall/js/game.js   # syntax check
```
...
- [ ] localStorage key `leaffall_best` persists best score across refreshes
