...
## Overview
Bomb Squad is a zero-dependency HTML5 canvas game (tap-to-defuse bomb reaction game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory bomb-squad
# Open http://localhost:8080
node --check bomb-squad/js/game.js   # syntax check
```
...
- [ ] localStorage key `bombsquad_best` persists best score across refreshes
