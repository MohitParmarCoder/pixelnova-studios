# Line Breaker — Deployment Guide

## Overview
Line Breaker is a zero-dependency HTML5 canvas game (ball-and-paddle brick breaker game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
line-breaker/
  ...
  js/
    ...
    game.js         # LineBreaker game logic
...
## Local Development
```bash
python3 -m http.server 8080 --directory line-breaker
# Open http://localhost:8080
node --check line-breaker/js/game.js   # syntax check
```
...
- [ ] localStorage key `linebreaker_best` persists best score across refreshes
