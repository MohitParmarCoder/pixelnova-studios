# Whack Mole — Deployment Guide

## Overview
Whack Mole is a zero-dependency HTML5 canvas game (3x3 grid whack-a-mole game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory whack-mole
# Open http://localhost:8080
node --check whack-mole/js/game.js   # syntax check
```
...
- [ ] localStorage key `whackmole_best` persists best score across refreshes
