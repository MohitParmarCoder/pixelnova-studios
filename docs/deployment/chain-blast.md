# Chain Blast — Deployment Guide

## Overview
Chain Blast is a zero-dependency HTML5 canvas game (chain-reaction bubble pop game). Deployment is a static file upload — no build step, no bundler, no npm install required.
...
## Local Development
```bash
python3 -m http.server 8080 --directory chain-blast
# Open http://localhost:8080
node --check chain-blast/js/game.js   # syntax check
```
...
- [ ] localStorage key `chainblast_best` persists best score across refreshes
